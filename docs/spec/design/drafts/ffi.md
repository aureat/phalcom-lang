...
| **F-3** | ~~Do native resources (sockets, files) force finalizers?~~ **CLOSED 2026-07-20 by [PDR-0005](../../../pdr/0005-resources-are-disposable-handles-not-finalized.md)** — no. Escapable handles + explicit `dispose` + a generation-tagged VM-side resource table, with `using` sugar lowering onto `ensure`. Collected-without-dispose is a reported leak, never a UAF; ADR-0050's "no finalizers" premise stands unamended. | — | Ruled. |
