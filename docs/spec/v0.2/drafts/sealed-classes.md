...
| # | Question | Notes |
|---|---|---|
| S-1 | **Unify the two "is sealed?" predicates?** | §2. `has_sealed` reads the attribute list (`attributes.rs:1540`); `extends` reads `VM::sealed_classes` (`class_decl.rs:364`). They disagree for `Option`/`Some`/`None`. Narrow today, but it is two representations of one property, and the bootstrap path populates only one. **Highest-value item here.** |
| S-2 | **Test the decorator's own headline enforcement.** | §1.3. No fixture extends a *user* `@sealed` class from another module. The `sealed_in_module != self.module` branch is exercised only via bootstrap-sealed core classes. |
