...
| Fact | Value |
|---|---|
| Last floor-affecting commit | **U-STRING** ([ADR-0049](../../../adr/accepted/0049-amend-floor-admit-string-byte-and-raw-write-primitives.md)) — `String#byteCount_`/`byteAt_(_)`/`slice_(_,_)` + `System.write_(_)`, floor **121 → 125**. Track total **73 → 125**: ADR-0023/0028/0036/0037 to 88, ADR-0039's three U-COLLTYPES phases to 109, ADR-0038 to 111, ADR-0045 to 112, ADR-0047 to 113, U-SCHED to 115, ADR-0052 to 117, M-ATTR-ROOT to 120, U-GC to 121, ADR-0049 to 125. _(This row was frozen at U-ERR/111 for five amendments — the same drift as DEFERRED CB-2. Re-derive from `invariants.rs`, never from this row.)_ |
| Primitive floor | **125** audited `(class, selector)` bindings · **110** distinct native fns · **22** floor-carrying classes (of **28** audited kernel classes) · **7** sacred selectors — see [`floor-census.md`](./floor-census.md) for the full enumeration and amendment-by-amendment history. **Do not quote these numbers** — they are a dated rendering of `invariants.rs::floor_census_matches_installed_bindings`, which is the source of record ([floor-census.md §1.3](./floor-census.md)). `VM::new()` installs **136**; the other 11 are `Fiber`'s, which no test audits (§1.4, DEFERRED CB-5). |
