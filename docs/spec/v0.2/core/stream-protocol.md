...
- **`Bytes`.** Every selector here takes or fills a `Bytes`, which does not exist yet
  ([`drafts/bytes.md`](../drafts/bytes.md)). That is a hard dependency for any implementation.
