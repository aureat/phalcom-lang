...
- **Realizes:** [ADR-0012](../../../../adr/0012-selector-signature-encoding-and-dispatch.md) (label-encoded selectors + inline-cache-ready dispatch); spec [messages-and-selectors §2–3](../../messages-and-selectors.md), [method-lookup §1](../../method-lookup.md), [object-model §7](../../object-model.md), [selectors.md](../../selectors.md).
...
- **Inline-cache population is deferred** — dispatch is built IC-*ready* here; the cache itself is a speed item, not part of the accepted decision. See [deferred-work](../../deferred-work.md).
- Canonical selector spelling was **amended to comma form** (`move(_,to,duration)`, [selectors.md](../../selectors.md)); ADR-0012's original colon spelling is superseded — decision and reasoning unchanged, only the string encoding. (The as-built `encode_selector` emits the `_:`/label`:` internal form seen in `method.rs`.)
