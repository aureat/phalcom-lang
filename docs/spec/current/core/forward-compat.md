...
> [TDR-0007](../../../decisions/accepted/0007-layered-exceptions-and-result.md) +
> [`error-handling.md`](../error-handling.md),
> [`open-questions.md`](../open-questions.md) §2 (numbers) / §8 (modules),
> [TDR-0009](../../../decisions/accepted/0009-tagged-value-enum.md).
...
- **Callables (U-CORE-3):** the call/`arity`/frame-token protocol must be
  ...
  (TDR-0012) — U-CORE-3's *surface* additions must not reintroduce a global-frame
...
- **Errors (U-CORE-6):** the raise/unwind path must be a *payload of the unified
  unwind* (TDR-0007 "one unwind primitive"), so a fiber can capture a propagating
  `Error` into its result slot rather than crashing the host. Do not special-case
  `throw` as host-process termination.
- **Value repr:** treat `Value` as **open for extension** (TDR-0009 already
...
## 2. Error mechanism ([TDR-0007](../../../decisions/accepted/0007-layered-exceptions-and-result.md))
...
- Building a **second, parallel** error channel that is not `Error`-subclass-based
  (TDR-0007: only `Error` subclasses are throwable; typed `on(_)` handlers require
  it).
...
- Modeling `Result` with a shape **incompatible** with `Option`'s
  abstract-root + two-subclasses layout (they must mirror, TDR-0007 / TDR-0006) —
...
- **Floor:** if U-CORE-6 needs a native raise primitive, that is an **TDR-0017
  amendment** (decisions.md Q2), not a silent `primitive!`.

## 3. Modules / imports (open-questions.md §8)

**(a) Coming (decided by [TDR — A module is a file; exports are public by default; imports are qualified, selective, or aliased](../../../decisions/retired/modules-as-files-with-public-by-default-imports.md)).**
...
foreclose the TDR-0024 scoping model.
...
## 4. Integer/Float split ([TDR-0022](../../../decisions/accepted/0022-numeric-surface-split-int-float-and-division.md), [TDR — Keep a single flat `Number` type backed by `f64`](../../../decisions/retired/number-as-flat-f64.md))

**(a) Coming (decided by [TDR-0022](../../../decisions/accepted/0022-numeric-surface-split-int-float-and-division.md)).** ADR-0005
...
- **Forward flag (revisit when TDR-0022 is implemented):** U-CORE-1's shipped
  `number_hash` masks the `f64` to **53 bits** of mantissa. This is a latent
  `hash`/`==` **soundness gap** once TDR-0022's exact bignum `Int` lands — two
...
| Non-local return is frame-local ⇒ fiber-local | [`concurrency.md`](../concurrency.md) §3; [TDR-0012](../../../decisions/accepted/0012-closure-upvalues-and-frame-token-return.md) |
| One unwind primitive; only `Error` throwable; `ensure` on any unwind | [TDR-0007](../../../decisions/accepted/0007-layered-exceptions-and-result.md) |
| `Result`/`Ok`/`Err` mirror `Option`/`Some`/`None` | [TDR-0007](../../../decisions/accepted/0007-layered-exceptions-and-result.md); [TDR-0006](../../../decisions/accepted/0006-option-as-abstract-with-some-none.md) |
| `import` semantics decided (file-as-module, public-by-default, qualified/selective/aliased) | [TDR — A module is a file; exports are public by default; imports are qualified, selective, or aliased](../../../decisions/retired/modules-as-files-with-public-by-default-imports.md); [`open-questions.md`](../open-questions.md) §8 |
| Current substrate flat `f64`; surface Int/Float split **decided** (bignum `Int` + `Float`, `/` true, `~/` floor) | [TDR-0022](../../../decisions/accepted/0022-numeric-surface-split-int-float-and-division.md); [TDR — Keep a single flat `Number` type backed by `f64`](../../../decisions/retired/number-as-flat-f64.md); [`open-questions.md`](../open-questions.md) §2 |
| `Value` is an extensible tagged enum | [TDR-0009](../../../decisions/accepted/0009-tagged-value-enum.md) |
...
| **C — stackful** | Give each fiber a real native stack (`corosensei`/`makecontext`-style switch). Yield crosses native frames. | Adds an `unsafe` stack-switch dependency; **directly collides** with the CROWN-JEWEL *stackful-fiber ⊗ moving/native-stack GC* hazard — every suspended fiber's native stack becomes a root the future collector must scan/relocate, weakening TDR-0008. | Constrains the GC design permanently. |
...
- §1(a)/§6: "`Fiber` = **new `Value` arm**" → **superseded**: `Fiber` is an
  ...
  TDR-0008/0010 and should be read as "a `FiberObject` on the heap."
