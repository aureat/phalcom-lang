...
> ([TDR-0020](../../../decisions/accepted/0020-string-interpolation-backslash-paren-sigil.md),
...
| `Block` | ✅ | ✅ | same as `Function` + `whileTrue(_)` | — | non-local-return surface protocol (mechanism exists, TDR-0012) | U-CORE-3 |
...
|---|:--:|:--:|---|---|---|---|
| `List` | ✅ | ✅ | `new` `length_` `at_` `set_` `push_` `toString` | `size` `at(_)` `add(_)` `each(_)` `at(_, put)` `map(_)` `filter(_)` `fold(initial,using)` `reduce(using)` `includes(_)` `isEmpty` | literal syntax `[a,b,c]` only (DEFERRED #6) | **U-STD — combinators landed**; literal syntax ratified ([TDR — List literals `[a, b, c]` desugar to `List` construction sends](../../../decisions/accepted/0025-list-literal-syntax.md)/[TDR-0028](../../../decisions/accepted/0028-collections-representation-and-literals.md)) |
...
Only `List` exists (TDR-0018). U-STD landed its combinator layer
...
parser work, DEFERRED #6), not part of U-STD's combinator scope. Per TDR-0018
...
governed by **[TDR-0007](../../../decisions/accepted/0007-layered-exceptions-and-result.md)**
(layered exceptions + Result) — so requirements-analysis **Q2 is partially
pre-decided**; U-CORE-6 must be read against TDR-0007, not designed from scratch.
...
TDR-0004 — no pending protocol; `Bool` itself stays ◐ pending `hash`/richer
...
- `Object#hash` is **present as of U-CORE-1** — a native floor primitive
  admitted by [TDR-0021](../../../decisions/accepted/0021-amend-floor-admit-hash-and-kernel-reflection.md),
...
| Error mechanism | [TDR-0007](../../../decisions/accepted/0007-layered-exceptions-and-result.md); `error.rs::RuntimeError` |
