...
| **U-CORE-6** errors | `Error` root + `MessageNotUnderstood` raise; reserve `Result`/`Ok`/`Err` | dispatch-ready; error surface **ratified** ([ADR-0031](../../adr/0031-error-handling-surface-syntax.md)) | [core/U-CORE-6](../../forge/units/U-CORE-6/ucore6.md) |
...
- **Default arguments** — [drafts/default-arguments.md](drafts/default-arguments.md) *(**chore discharged 2026-07-15, DEFERRED CB-4**: `experimental/default-arguments.md` was retired rather than reconciled — its body advocated the caller-side desugar its own banner permanently forbade, and the draft already carried its content correctly. ADR-0043 stands.)*
