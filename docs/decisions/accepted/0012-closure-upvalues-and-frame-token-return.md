# TDR-0012 — Open/closed upvalues and frame-token non-local return

- Legacy record: `ADR-0013`
- Ratified: 2026-07-11

- Status: Accepted
- Date: 2026-07-11
- Related: `docs/spec/current/blocks.md` §5; `docs/spec/current/object-model.md` §4; [TDR-0005](./0005-function-as-abstract-callable-root.md); [TDR-0008](./0008-handle-arena-heap.md)

## Context

Blocks are the keystone construct — a block, a lambda, a method body, and a getter
body share one closure representation ([Blocks](../../spec/current/blocks.md), [TDR-0005](./0005-function-as-abstract-callable-root.md)).
...
- **Escaping blocks.** A block may outlive the frame in which it was created
  ([Blocks §5](../../spec/current/blocks.md)); captured variables must remain live and, where
...
- **Non-local return.** `return` inside a block unwinds to its *home method* frame,
  ...
  ([Blocks §5](../../spec/current/blocks.md), [Object Model §4](../../spec/current/object-model.md)).
...
- One `ClosureObject` is shared by `Block` and `Method` ([TDR-0005](./0005-function-as-abstract-callable-root.md));
  upvalue cells live in the heap ([TDR-0008](./0008-handle-arena-heap.md)) so a closed
...
- Escaping blocks and shared mutation of captured variables both work, matching
  [Blocks §5](../../spec/current/blocks.md); an inner block that mutates a captured `var` is
...
- Non-local return is safe by construction: the generation check converts "return to
  ...
  form is expression-only and cannot carry `return` ([Blocks §2](../../spec/current/blocks.md)),
  which keeps the safe cases the common cases.)
- One closure representation means `Fiber`/`Future` ([Concurrency](../../spec/current/concurrency.md))
...
- Open→closed promotion must interact correctly with the heap ownership model
  ([TDR-0008](./0008-handle-arena-heap.md)): the upvalue cell's lifetime is the
  closure's, not the frame's.
- The frame token also unifies with `throw` and fiber `abort` as one stack-unwinding
  primitive ([TDR-0007](./0007-layered-exceptions-and-result.md)).
