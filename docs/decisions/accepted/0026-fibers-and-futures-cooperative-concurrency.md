# TDR-0026 — Fibers and Futures: cooperative concurrency on a restricted re-entrant loop

- Legacy record: `ADR-0030`
- Ratified: 2026-07-12
...
- Related: [`docs/spec/current/concurrency.md`](../../spec/current/concurrency.md);
  ...
  [TDR-0008](./0008-handle-arena-heap.md) (handle heap);
  [TDR-0009](./0009-tagged-value-enum.md) (tagged `Value`);
  [TDR-0012](./0012-closure-upvalues-and-frame-token-return.md) (frame-token return);
  [TDR-0007](./0007-layered-exceptions-and-result.md) (one unwind primitive);
  [TDR-0017](./0017-freeze-vm-blessed-primitive-floor.md) (floor)
...
A `FiberObject` is one more arena variant (`Object::Fiber`, [TDR-0008](./0008-handle-arena-heap.md))
reached through `Value::Obj(ObjRef)`, exactly as native `List` is — **no new
`Value::Fiber` arm** ([TDR-0009](./0009-tagged-value-enum.md)). It owns its own
...
- **Works:** any fiber whose body uses pure Phalcom sends and **inlined** control
  flow. The [TDR-0016](./0016-sacred-selector-inliner-and-override-guard.md) inliner
...
Once `self.frames` is the *current* fiber's vector, [TDR-0012](./0012-closure-upvalues-and-frame-token-return.md)'s
...
non-matching. Likewise the [TDR-0007](./0007-layered-exceptions-and-result.md)
...
- **GC design stays open.** No native fiber stacks means nothing new for a moving
  collector to scan or relocate; [TDR-0008](./0008-handle-arena-heap.md)'s
...
- **New floor surface (ADR-0019 amendment).** The Fiber/Future primitive set —
  ...
  [TDR-0017](./0017-freeze-vm-blessed-primitive-floor.md) amendment convention (as
  [TDR-0018](./0018-kernel-list-native-array-protocol.md)/[TDR-0021](./0021-amend-floor-admit-hash-and-kernel-reflection.md)
...
- **C — stackful coroutines.** Give each fiber a real native stack
  ...
  directly weakening [TDR-0008](./0008-handle-arena-heap.md). The power is not worth
...
- **Resumable (Smalltalk) suspension for failures.** Out of scope; error
  propagation is terminating per [TDR-0007](./0007-layered-exceptions-and-result.md).
