# TDR-0032 — Amend the frozen floor — admit `Block#on`/`Block#ensure` (error handling)

- Legacy record: `ADR-0038`
- Ratified: 2026-07-12

- Status: Accepted
- Date: 2026-07-12
- Related: [TDR-0017](./0017-freeze-vm-blessed-primitive-floor.md) (the frozen floor);
  [TDR-0027](./0027-error-handling-surface-syntax.md) (the ratified error surface);
  [TDR-0007](./0007-layered-exceptions-and-result.md) (error model — handling is a `Block` protocol);
  [TDR-0031](./0031-amend-floor-admit-error-root.md) (the **raise** side — `Error#message`/`raise`);
  [TDR-0021](./0021-amend-floor-admit-hash-and-kernel-reflection.md) (amendment precedent);
  [`../../forge/units/U-ERR/err-plan.md`](../forge/units/U-ERR/plan.md) (DEC-ERR-A)

## Context

[TDR-0027](./0027-error-handling-surface-syntax.md) ratified `try`/`catch`/`on`/`ensure`
as **1:1 sugar over the `Block` protocol** `blk.on(ErrorClass){ e => … }` /
`blk.ensure{ … }` ([TDR-0007](./0007-layered-exceptions-and-result.md)). But those two
...
[TDR-0017](./0017-freeze-vm-blessed-primitive-floor.md) §1 derivability test, so
admitting them is a **floor amendment**, not an ordinary commit.

[TDR-0031](./0031-amend-floor-admit-error-root.md) admits the **raise** side
...
[decisions.md Q2](../../forge/units/U-CORE-0/decision-register.md) reserved. `U-ERR/plan.md` DEC-ERR-A
flags it as the one gate that must clear before the primitives merge.

## Decision

**Amend [TDR-0017](./0017-freeze-vm-blessed-primitive-floor.md) to admit exactly two
...
- **Fiber-safe by construction.** The snapshot/restore is **length-relative**, so it
  stays fiber-local when U-FIBER lands ([TDR-0026](./0026-fibers-and-futures-cooperative-concurrency.md)
