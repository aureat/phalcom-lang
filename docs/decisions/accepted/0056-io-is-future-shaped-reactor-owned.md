# TDR-0056 — IO is `Future`-shaped and reactor-owned; the reactor is built before the IO surface

- Legacy record: `PDR-0004`
- Ratified: 2026-07-20

- Status: Accepted
- Date: 2026-07-20
- Related: [TDR-0055](./0055-no-user-visible-threads-fibers-and-isolates.md) (no user-visible
  threads — constrains how the filesystem half may be implemented),
  [TDR-0026](./0026-fibers-and-futures-cooperative-concurrency.md) (`Fiber` +
  `Future`, the parking mechanism this rests on),
  [E004](../errors/E004-await-cannot-suspend.md) (**FIXED** `f479189` — the precondition),
  [system.md](../../spec/current/system.md) (`System.sleep(_)` was the open timer question this closes),
  [ffi.md](../spec/current/drafts/ffi.md) §4 (the object-graph boundary the worker pool must respect)
...
- **`Resource#close` returns `Result` synchronously**
  ([TDR-0057](./0057-resources-are-disposable-handles-not-finalized.md) §3b). Over an unbuffered
...
[system.md](../../spec/current/system.md) records `sleep` as "still open — U-SCHED deliberately splits
