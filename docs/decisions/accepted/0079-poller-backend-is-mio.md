# TDR-0079 — The poller backend is `mio`, wrapped once at the reactor seam; syscalls try first and register second

- Legacy record: `PDR-0016`
- Ratified: 2026-09-12
...
- Date: 2026-07-20
- Related: [TDR-0056](./0056-io-is-future-shaped-reactor-owned.md) §3 (pollable descriptors
  get "a real reactor (`epoll`/`kqueue`/IOCP)"),
  [TDR-0055](./0055-no-user-visible-threads-fibers-and-isolates.md) §3 (single VM thread —
  the poller must not add a second),
  [TDR — The network surface: TCP is poller-backed and `Future`-shaped, DNS rides the pool, endpoints are address-plus-port](../proposed/network-surface-tcp-dns-endpoints.md) (the surface this serves; its
...
- Development is macOS-first (`kqueue`), but the tree is *unix*-first, not Darwin-first —
  [`impl/filesystem.md`](../forge/units/U-FS/implementation-spec.md) §2.1 gates on
