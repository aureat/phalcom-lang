# TDR-0005 — `Function` as the abstract root of the callable tower

- Legacy record: `ADR-0006`
- Ratified: 2026-07-11

- Status: Accepted
- Date: 2026-07-11
- Related: [TDR-0003](./0003-introduce-behavior-kernel-class.md); `docs/spec/current/functions.md`, `docs/spec/current/blocks.md`

## Context

The [Blocks](../../spec/current/blocks.md) spec (§7) stated "a method *is* a `Block`." That
...
Introduce `Function` as an **abstract** kernel class ([Object Model](../../spec/current/object-model.md)):
...
- One call protocol, one closure representation (`ClosureObject`), two owners.
  `Fiber` and `Future` ([Fibers & Futures](../../spec/current/concurrency.md)) take a
  `Function` as their unit of work without caring whether it is a block or a bound
  method.
- [Blocks §7](../../spec/current/blocks.md) is amended: "sibling under `Function`," not "is a
...
- Requires first-class blocks to land (a `Value::Block` arm plus `Closure` /
  ...
  [Implementation Status](../../forge/spec-status.md).
