# TDR-0001 — Record architecture decisions

- Legacy record: `ADR-0001`
- Ratified: 2026-07-11
...
We will record significant architecture decisions as TDRs in `docs/decisions/`,
using Nygard's Context/Decision/Consequences format, one decision per file. A TDR is
...
- A small ongoing cost: decisions must be written up. We accept this for
  decisions that clear the "warranted" bar above; trivial choices need no TDR.
- TDRs complement the layered knowledge system in `CLAUDE.md`: the graphify
  graph answers *structure*, TDRs answer *intent*.
