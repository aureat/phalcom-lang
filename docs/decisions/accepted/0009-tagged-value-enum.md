# TDR-0009 — `Value` is a 16-byte explicit tagged representation with a private `Nil` sentinel

- Legacy record: `ADR-0010`
- Ratified: 2026-07-11

- Status: Accepted (amended by [TDR-0022](./0022-numeric-surface-split-int-float-and-division.md) and Spec 2: 16-byte Value Representation)
- Date: 2026-07-11
- Related: `docs/spec/current/object-model.md` §3; `docs/spec/current/values-and-absence.md` §2; [TDR — Keep a single flat `Number` type backed by `f64`](../retired/number-as-flat-f64.md); [TDR-0008](./0008-handle-arena-heap.md); [TDR-0022](./0022-numeric-surface-split-int-float-and-division.md)
...
> **Numeric arm amended (2026-07-12) by [TDR-0022](./0022-numeric-surface-split-int-float-and-division.md).**
...
but the VM needs one in-register representation for all of them. [TDR — Keep a single flat `Number` type backed by `f64`](../retired/number-as-flat-f64.md)
...
- **`nil` is private (Invariant 4).** The VM keeps a `nil` for uninitialized slots
  ...
  Absence is `Option`.
- Heap objects are now handles, not pointers ([TDR-0008](./0008-handle-arena-heap.md)),
...
- **A surface `nil` value** (JavaScript/Wren style). Rejected: it reintroduces the
  ...
  Invariant 4); absence is `Option` ([TDR-0006](./0006-option-as-abstract-with-some-none.md)).
...
- **`Rc`-carrying object arm.** Superseded by the handle heap ([TDR-0008](./0008-handle-arena-heap.md));
