...
| [0011](0011-admit-bytes-native-octet-buffer.md) | Admit `Bytes`: native octet buffer arm, ten floor primitives, and the container bulk-op admission posture (amends ADR-0019, floor 137 → 147) | **Proposed** | *amends* ADR-0019 | | ❌ not ratified — do not design against it (README rule 5). Spec drafted at [`bytes.md`](../spec/v0.2/core/bytes.md), normative upon ratification; unblocks [`stream-protocol.md`](../spec/v0.2/core/stream-protocol.md) §9's hard dependency |
| [0012](0012-numeric-tower-implementation-and-floor-amendment.md) | The numeric tower lands: `Int`/`Float` implementation rulings and the floor amendment (137 → 153) | **Proposed** | *amends* ADR-0019; extends ADR-0024 (**not** superseded — its §A restates ADR-0024 as settled input) | | ❌ not ratified — do not design against it (README rule 5). Spec at [`numeric-tower.md`](../spec/v0.2/core/numeric-tower.md), normative upon ratification. ADR-0024 has been Accepted-and-unbuilt since 2026-07-12 with no owning unit; this record supplies what it left open. **Composes with 0011** — both amend ADR-0019 off the same measured 137 over disjoint classes, so whichever ratifies second rebases (both ⇒ **163**; ruling 21). Six open questions; Q-1 (`~/` on `Float`) and Q-2 (`Int.new(2.7)`) are genuinely unruled by ADR-0024 |
| [0013](0013-path-is-bytes-backed-filesystem-surface.md) | `Path` is bytes-backed, not a `String`; the filesystem surface (`Fs`/`DirEntry`/`Metadata`/`OpenMode`; +1 `Bytes#utf8Lossy_`) | **Proposed** | | | ❌ not ratified — do not design against it (README rule 5). Spec at [`filesystem.md`](../spec/v0.2/core/filesystem.md) — split authority: its §4 `File` table encodes the already-ratified PDR-0005 §7, the rest waits on this record. Depends on 0011 (`Bytes`); floor delta enumerated in ruling 10, censused at impl under 0012 ruling 21's rebase discipline. Impl double-blocked: ratification + the reactor (0004 §2) |
