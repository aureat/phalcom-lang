...
| [0006](0006-repl-completeness-is-a-parser-signal.md) | REPL completeness is a parser signal; the lexer reports unterminated modes | Accepted | | | ❌ ruled 2026-07-20, unimplemented — §2's co-emission at `phalcom-ast/src/parser.rs:146-147` is not written, so a bare `/* …` still submits |
| [0007](0007-bounded-call-depth-and-native-reentrancy.md) | Bounded call depth and native re-entrancy: two counters, one error | Accepted | | | ❌ ruled 2026-07-20, unimplemented. Closes the overlay's "resource limits unspecified" gap; infinite recursion is still a measured OOM hang |
