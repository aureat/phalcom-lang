...
| [0069](0069-resources-are-disposable-handles-not-finalized.md) | Native resources are closeable handles with a generation-tagged table; no finalizers | Accepted (**revised same-day 2026-07-20** — interface ratified: `close` not `dispose`, returns `Future<Result>`, `Resource` root class, `using` withdrawn; see its header) | | | ❌ ruled 2026-07-20, unimplemented. Selector surface ratified in §7. Closes ffi.md F-3; defers the multi-axis protocol problem to [`docs/deferred/io-protocol-axes-need-mixins.md`](../deferred/io-protocol-axes-need-mixins.md) |
