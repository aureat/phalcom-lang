# TDR — Phalcom language intelligence is an in-process `phalcom-lsp` server

- Legacy record: `ADR-0056`
- Proposed: 2026-07-13

- Status: Proposed
- Date: 2026-07-13
- Related: [TDR-0011](../accepted/0011-selector-signature-encoding-and-dispatch.md)
  (selectors are comma-form label-encoded symbols — the symbol-index key);
  [TDR-0014](../accepted/0014-hand-written-lexer-and-recursive-descent-parser.md)
  (`phalcom-ast` is the standalone, VM-free front end this server embeds);
  [TDR-0023](../accepted/0023-external-internal-parameter-names.md) (external labels the
  completion snippets render); [TDR — A module is a file; exports are public by default; imports are qualified, selective, or aliased](../retired/modules-as-files-with-public-by-default-imports.md)
...
3. **Selectors are comma-form label-encoded symbols** ([TDR-0011](../accepted/0011-selector-signature-encoding-and-dispatch.md)):
...
- **Definitions:** for each `.ph` file's `Parse.program`, walk `ClassDef →
  ...
  [TDR-0023](../accepted/0023-external-internal-parameter-names.md); `Getter`→`name`,
...
- **More TypeScript intelligence in the extension** (parse the buffer in JS).
  ...
  — exactly the drift ([TDR-0014](../accepted/0014-hand-written-lexer-and-recursive-descent-parser.md))
