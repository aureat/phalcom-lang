...
---
# CLIT001.C2.P1 — repl session and cells

Implements P1-repl-architecture.md **§D1** (one session module), **§D10** (cell boundaries unwind), and
...
[§00](../C1-repl-foundation/P2-repl-branch-integration.md); the split is why this stage is written with its core-side
...
### 1.3 Rejected, per P1-repl-architecture.md §D1 — do not revisit

- **Replaying the accumulated buffer per cell.** Side effects re-fire; O(n²).
  (`:reload` in [§07](../C4-repl-operations/P1-repl-commands.md) replays *by explicit user request*. That is not
...
Mirrors CPython's `"single"` mode. Rejected (P1-repl-architecture.md §D3): Lua-style `return`-prepending
...
The cell loop calls `VM::unwind_cell()` ([§01 §3.1](../C1-repl-foundation/P3-repl-compiler-wiring.md)) at every cell
...
after a failed cell and must not assume the VM cleaned up. This is stated in P1-repl-architecture.md
...
`#[allow(dead_code)]` from [§01](../C1-repl-foundation/P3-repl-compiler-wiring.md) is **deleted** in this stage.
