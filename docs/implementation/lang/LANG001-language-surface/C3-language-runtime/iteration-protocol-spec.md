# LANG001.C3 — Iteration protocol spec
...
| receiver of `for` does not answer `iterate(_)` | ordinary **`doesNotUnderstand`** miss → surface `MessageNotUnderstood` (U-CORE-6) at the first `_coll.iterate(None)` send | dispatch, [error surface of U-CORE-6](../U-CORE-6/ucore6.md) |
