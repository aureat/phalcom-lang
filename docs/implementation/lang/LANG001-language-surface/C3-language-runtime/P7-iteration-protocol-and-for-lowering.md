...
# LANG001.C3.P7 — iteration protocol and for lowering
...
|---|---|---|---|
| **DEC-ITER-A** ✅ **RESOLVED (user, 2026-07-12): (A) — U-STD follow-on.** | **Migrate the combinators now?** `each`/`map`/`filter`/`reduce` currently sit on `size`/`at`; ADR-0035 §5 wants them as `.ph` defaults over `iterate`/`iteratorValue`. | **(A)** leave them as-is; U-ITER ships protocol + `for` only; a **U-STD follow-on** rewrites the combinators onto the protocol. **(B)** rewrite them here. | **(A)** — U-ITER's `core.ph` edit is limited to `List#iterate`/`iteratorValue`; the combinator migration is a separate small U-STD follow-on (add a `DEFERRED.md` pointer). Both mechanisms are correct in parallel meanwhile; keeps the write-set small vs the live U-CORE editors. |
