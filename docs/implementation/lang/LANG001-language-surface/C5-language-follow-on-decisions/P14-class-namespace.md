...
`err-plan.md`'s preconditions were re-verified 2026-07-19 against the tree. **Every line number in
`err-plan.md` has drifted** (U-BINDINGS rewrote `vm/mod.rs` and `class_decl.rs` this week), and the
census found **two more tables and one compiler field** that `err-plan.md` does not name and that
...
> commits sit between it and the SHA `err-plan.md` cites; all three are documentation-only, so every
...
| # | `err-plan.md` claim | Verdict | Evidence |
|---|---|---|---|
...
`err-plan.md` names four. Two more are keyed on class-name `Symbol` and are walked **interleaved
...
This is the single largest divergence from `err-plan.md`'s sizing. Budget **+8–10 sites**.

### 1.3 `Compiler::current_class` is the seventh site and `err-plan.md` misses it
...
Good news `err-plan.md` does not claim. `ClassLayout` (`vm/mod.rs:27`) derives only
...
`err-plan.md`, PDR-0001, and the U-CLASSCLOSE plan all cite `class_decl.rs:424` as the sole
...
`err-plan.md` treats `create_class`/`create_single_class` as an API-surface problem. Measured:
...
- The redundancy is worth *pinning* rather than exploiting: §11 adds an invariant test asserting
  ...
  of a re-derivation.
- `err-plan.md` §3.1 already rules "do not collapse them" for the same reason. This section only
...
`err-plan.md` frames the VM half as a mechanical key swap. It is, for 58 of the 62 sites. Four are
...
This is the unit's real risk, and `err-plan.md` does not mention it. [§4](#4-superclass-resolution--the-load-bearing-question)
...
**A bare tuple is rejected** for the reason `err-plan.md` gives (62+ positional sites), and the
...
`err-plan.md` leaves this open. It cannot stay `Symbol → Symbol`:
...
**Do not harden the walks.** `err-plan.md`'s rubric and `DEC-CTOR-H` both apply: this guard is
...
Rejected alternatives, per `err-plan.md` §3.4 and PDR-0002: a separate `ClassKey → SourceRange`
...
- kernel sealing still resolves to the core module after the re-key —
  `sealed_classes` for `Option`/`Some`/`None` (per `err-plan.md`'s rubric);
...
1. **The unit is larger than `err-plan.md` prices it, and the extra work is not the boring kind.**
