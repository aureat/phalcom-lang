...
number. Complements, doesn't replace, the status of record (`u0-state.md` +
...
- [x] **U0** — [as-built](units/U0/u0.md) — stand up `verify.sh`, golden corpus, invariant harness; every later unit gates on this.
- [x] **U1** — [as-built](units/U1/u1.md) — slotmap handle/arena heap + tagged `Value` (ADR-0009/0010).
- [x] **U2** — [as-built](units/U2/u2.md) — parallel metaclass rule + `Behavior` kernel + `verify_invariants()` (ADR-0002/0003).
- [x] **U3** — [as-built](units/U3/u3.md) — label-encoded selector symbols replace arity-only dispatch (ADR-0012).
- [x] **U4** — [as-built](units/U4/u4.md) — first-class blocks/closures, open/closed upvalues, frame-token infra (ADR-0013 groundwork).
- [x] **U5** — [as-built](units/U5/u5.md) — control flow lowered to ordinary sends via the guarded sacred-selector inliner (ADR-0018).
- [x] **U6** — [as-built](units/U6/u6.md) — absence → first-class `Option`, no surface nil, `let`/`var` (ADR-0007/0014/0021).
- [x] **U7** — [as-built](units/U7/u7.md) — fixed per-class slot layout, `construct`, class-side stored static fields (ADR-0011/0017).
- [x] **U8** — [as-built](units/U8/u8.md) — `doesNotUnderstand`/`perform`/`respondsTo` + `Message` reification (ADR-0012 amendment).
- [x] **U9** — [as-built](units/U9/u9.md) — trailing `*name` rest params collapse extra args into a `List`.
- [x] **U10** — [as-built](units/U10/u10.md) — non-local `return` inside a block via frame-token unwind (ADR-0013).
- [x] **U11** — [as-built](units/U11/u11.md) — `Bool` split into abstract root + `True`/`False` singletons (ADR-0004).
- [x] **U-FE** — [as-built](units/U-FE/fe-lexer-recursive-descent-parser.md) — hand-written lexer + recursive-descent/Pratt parser, replacing logos/LALRPOP (ADR-0016).
- [x] **U-LEX** — [as-built](units/U-LEX/lexical-surface-syntax=delta.md) — block comments, digit separators, newline suppression, `\(expr)` interpolation, `??`/`?.`.
- [x] **U-LIST** — [as-built](units/U-LIST/u24-list.md) — native `Vec<Value>` kernel `List` + thin `.ph` protocol (ADR-0019/0020).
- [x] **U-STD** — [as-built](units/U-STD/u-21-std.md) — pure-`.ph` `Option`/`List` combinator layer over the frozen floor.
...
- [x] **U-CORE-2** — [as-built](units/U-CORE-2/ucore2.md) — `0da64d6` Some-lift one-armed `ifTrue`/`ifFalse`, closes the half-`Option` divergence.
- [x] **U-CORE-1** — [as-built](units/U-CORE-1/ucore1.md) — `03764e3` kernel reflection: `hash`, `isA`, `Behavior.name`/`.methods` + invariant substrate (floor 73→80).
- [x] **U-CORE-3** — [as-built](units/U-CORE-3/ucore3.md) — `10ebd06` callables/`Block` reflection surface + `BoundMethod` value.
- [x] **U-CORE-4** — [as-built](units/U-CORE-4/ucore4.md) — `2061795` per-type `toString` overrides, unified native print path.
- [x] **U-CORE-5** — [as-built](units/U-CORE-5/ucore5.md) — `bc161fb` shared collection-protocol contract; `List` as reference impl; structural `#==`/`!=`.
- [x] **U-CORE-6** — [as-built](units/U-CORE-6/ucore6.md) — `85c4e1d` `Error` root + `MessageNotUnderstood`, wired into the unified unwind.
- [x] **U12** — [plan](units/U12/u12.md) — *(decision closed, no code)* `f16b58a` DEC-U12=A: keep flat `f64` `Number`, Integer/Float split deferred.
- [x] **U17** — [plan](units/U17/u17.md) — *(decision closed, no code)* `0ff3239` ADR-0044: formalize `None`/`Option` bootstrap-cycle avoidance; niche-encoding deferred.
...
- [x] **U-INH** — [plan](units/U-INH/u27-inh.md) — `0e920ab`/`3248b05` single inheritance: `extends`, parallel-metaclass repair, `SuperSend` + super-construct chaining.
...
- [x] **U13** — [plan](units/U13/u13.md) — `5d84ad8` ADR-0041 hierarchy-stability policy: sealed, no runtime superclass reassignment; traits/MI ruled out.
...
- [x] **U-ITER** — [plan](units/U-ITER/u26-iter-plan.md) · [spec](units/U-ITER/u26-iter-spec1.md) — cursor protocol (`iterate`/`iteratorValue`) + `for`/`break`/`continue` (ADR-0035). Landed in 4 steps, `aa964bb`→`0142c7a`.
- [x] **U-FIBER** — [plan](units/U-FIBER/plan.md) — *(see §6, lands interleaved with U-ITER)*
- [x] **U-COLL** — [plan](units/U-COLL/literals-tuple-map.md) — `1274504`/`5bc31e8`/`dc9eab0` literal sugar: `[…]`/`(a,b)`/`{k:v}` desugar to List/Tuple/Map construction sends (ADR-0029/0032).
- [x] **U-ITER-FIX** — [plan](units/U-ITER-FIX/u25-iter-fix-plan.md) — four loop-control follow-ons: `9288ad5` materialized-block break/continue trap, `ac4f721` bare-`while` break/continue, `08a323b` fresh loop-var cell per iteration, `b566e6b` jump-helper dedup.
- [x] **U-COLLTYPES** — [plan](units/U-COLLTYPES/map-set-tuple-range.md) — native `Map`/`Set` (`be8426e`), `Tuple` (`2d140f0`), `Range` (`f934cf1`), map-literal wire (`10e1715`) — ADR-0032/0039 floor amendment.
- [x] **U-STD** — *(see §1 — List#each migrated onto cursor protocol here, `c35171a`, alongside this track)*
- [x] **U-ITERABLE** — [plan](units/U-ITERABLE/u23-iterable-plan.md) — bare-cursor Route B (raw index cursor + `None` end-sentinel, no per-step `Some` allocation) + kernel `Iterable` root (`core.ph:646`) hoisting `each`/`map`/`filter`/`reduce` (ADR-0048). Golden suite rebaselined off the pre-Route-B Option-wrapped-cursor protocol: `list_wren_iterate_cursor_protocol`, `map_wren_cursor_roundtrip`, `range_cursor_protocol_direct` (bare `0`/`1`/… + `None` sentinel, no `Some.new(_)`/`.unwrapOr`/`.isSome`), plus 3 negative fixtures (`map_wren_iterate_not_int/not_num`, `range_iterate_wrong_cursor_type`) updated from the old `does not understand 'map(_)'` dNU to the new arithmetic rejection `Expected String, got number` — `cursor + 1` on a non-numeric cursor is the natural error now, not a `.map(_)` dispatch on a non-`Option`. `./scripts/verify.sh` green for all of these. Gate-clean for Route B itself — **unrelated** pre-existing gap remains in `indexing`/`indexing_negative` (bracket-selector method-definition syntax `[i] { ... }` and empty-bracket calls `xs[]` don't parse at all; predates this unit, `26747d0`) — tracked separately, not a U-ITERABLE regression.
- [x] **U-SEQ** — [plan](units/U-SEQ/u22-seq-plan.md) · [spec](units/U-SEQ/u22-seq-spec.md) — historical U-SEQ record retained: `606829e` combinator breadth + lazy view classes (steps 1–2), `e6e767a` sugar wiring (step 3, **DEC-SEQ-A = Branch A**), `838244a` lazy-semantics comments. **Current D.1/E status:** direct collection transforms are eager, predicate queries use labeled `where:`, Map pair traversal uses `map.entries.each`, and laziness starts at explicit `.iter` pipelines. Focused sequence and D.1 lanes are green; deferred spread fixtures remain separate pending work. **Open:** no `fe-lexer-recursive-descent-parser.md`; spec §5's `all_generator_raises.ph` fiber fixture was never written (only `concurrency/each_generator_raises.ph` exists).
- [x] **U-STRING** — [plan](units/U-STRING/u22-string.md) — `0bae56d` `ArgumentError` boundary-guard class · `3a37b19` raw primitives + `System.rawWrite` (ADR-0049) · `bd3f492` String protocol + write funnel (steps 3–6) · `18b7c9a` core rework (`split()` fix, UTF-8 `codePointAt`) · `3b2dd97`/`a21367d` `trimStart`/`trimEnd` charset iteration + stop-on-mismatch · `64de2bf` negative-lane guards + `bytes`/`codePoints` golden. `String` protocol at `core.ph:84-360`, `StringByteSequence`/`StringCodePointSequence` at `:363`/`:388`. `string`/`strings`/`strings_negative` lanes green. **Open:** no `fe-lexer-recursive-descent-parser.md`.
...
- [x] **U-FUTURE (Slice A)** — [plan](../work/pending/fiber-schedule/future/plan.md) — `f0d128a` settle-once `Future` state machine, pure `.ph`, no scheduler dependency.
...
- [x] **U-PERF** — [work-1](units/U-PERF/perf-1.md) · [work-2](units/U-PERF/perf-2.md) — session ledgers for the perf investigation track itself (`81d494c`, `54dae3a`), including the ideas that failed and why.

## 12. Naming conventions (opportunistic, no urgency)

- [x] **U-NATIVE-MARKER** — [plan](units/U-NATIVE-MARKER/u32-native-marker.md) — `3e362e2` mechanical rename, native/private primitive `raw*` prefix → trailing `_` suffix (Wren convention).
