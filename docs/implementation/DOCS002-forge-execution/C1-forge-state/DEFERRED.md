...
**The defect.** [TDR-0020](../../../decisions/accepted/0020-string-interpolation-backslash-paren-sigil.md) desugars
...
[TDR-0013](../../../decisions/accepted/0013-object-default-tostring.md) governs. This is the
...
[STATE.md](STATE.md) and `docs/decisions/STATUS.md`'s ADR-0036 row). The blocker is gone; the
...
| Source | Count |
|---|---|
| `docs/spec/current/core/floor-census.md:36` §1.1 "Installed `(class, selector)` bindings" | **113** |
| `docs/spec/current/core/floor-census.md:665` §7 (audit-hook prose) | **117** |
...
([TDR-0011](../../../decisions/accepted/0011-selector-signature-encoding-and-dispatch.md)) and there are no
default arguments ([TDR-0036](../../../decisions/accepted/0036-no-default-arguments-keep-selector-identity-pristine.md)),
...
[TDR-0039](../../../decisions/accepted/0039-destructuring-bindings.md) shipped only irrefutable
destructuring). See [`drafts/sealed-classes.md`](../spec/current/drafts/sealed-classes.md) §S-1.
...
`docs/spec/current/experimental/default-arguments.md` is 40 lines and self-contradictory:
...
readers skip. `docs/spec/current/deferred-work.md:163` already logs this reconciliation as an
...
[`drafts/default-arguments.md`](../spec/current/drafts/default-arguments.md) §8 (DA-1/DA-2),
...
|---|---|---|
| `phalcom-core/src/primitive/block.rs` `block_ensure`'s temp-root special-case — found in [U-GC spine review](../logs/2026-07-20-u-gc-spine-review.md) §2 | robustness — fragile against silent drift, not a live bug | `Err(PhError::Runtime(RuntimeError::Raise { error, .. })) => vm.push_temp_root(*error), Err(_) => {}` roots the pending `ensure` outcome across the re-entrant cleanup call only for `Ok` and `Raise`. Checked every `RuntimeError` variant on 2026-07-20: `Raise` is the only one that carries a `Value`, so today's catch-all is correct. But it is a `match … { _ => {} }`, not `trace_object`'s exhaustive-match-no-wildcard discipline — a future `RuntimeError` variant that adds a `Value` field compiles silently without rooting it, and the collector would free it out from under `ensure`. Consider a helper (`RuntimeError::carried_value(&self) -> Option<Value>`) that forces every variant to answer, mirroring the discipline `heap/trace.rs` already uses for `Object`. |
| `docs/spec/current/drafts/decorators-dispatch-observability.md` (Runtime tier: `aroundSend`/`Invocation`) — flagged in [U-GC plan](units/U-GC/plan.md) §3.8, temp-root machinery landed | unit scope — forward obligation, not yet a defect | **M-RUNTIME: no memory-management note has been written for the unbuilt Runtime decorator tier.** U-GC's collector (`vm/mod.rs:196-197` `temp_roots`/`push_temp_root`/`pop_temp_root`) discharges every *currently live* re-entrant-handle hazard (the §3.4 audit found the intersection with real alloc sites empty). But `aroundSend` — when built — will allocate an `Invocation` object per intercepted send and then re-enter the interpreter to run the wrapped call, which is exactly the §3.4 hazard shape (a fresh handle held in a Rust local across a re-entrant send) that the temp-root API exists to protect. `grep -rln "aroundSend\|Invocation" phalcom-core/src` currently only matches `compiler/attributes.rs` (decorator-name recognition, not the tier itself) — the Install/Dispatch/Runtime/Layout tiers are all unbuilt. **Precondition to un-defer:** whoever implements the Runtime tier must wrap the `Invocation` allocation and the re-entrant dispatch it drives in `push_temp_root`/`pop_temp_root`, and memory-management.md should gain a short note citing this obligation so it isn't rediscovered by audit. Not a bug today — nothing exercises the hazard, since the tier does not exist. |
| `phalcom-repl/src/{highlighter,completer}.rs` · `phalcom-repl/Cargo.toml:12` (ruled 2026-07-20, [TDR-0061](../../../decisions/accepted/0061-defer-lsp-backed-repl-surface.md)) | unit scope — deferred pending ADR ratification | **U-REPL Stage 6 — the LSP-backed REPL surface — is unbuilt and deferred.** U-REPL's impl spec mandates it with named entry points and an explicit *"Call it. Do not port it."* (`units/U-REPL/impl/06-surface.md:40-43` for §S5's L2 semantic-token pass; `surface.md:58-61` for reusing the LSP's `ReceiverKind` completion split). None of it exists: `grep -rn phalcom_lsp phalcom-repl/src/` returns **zero** hits, the highlighter implements only L1 + L3, and the completer classifies position by `rfind('.')` string slicing. **The blocker is not effort — it is that ADR-0056, which authorizes the `phalcom-lsp` crate at all, is still `Proposed`.** Building Stage 6 would ratify it by fait accompli. The dependency **stays** in `Cargo.toml` (removing it deletes a mandated write-set target and turns a deferral into a silent abandonment). §S6's idle debounce is unbuilt for the same reason. **Precondition to un-defer: ratify ADR-0056, or supersede it with a PDR.** Related corrections already applied: `highlighter.rs`'s module doc no longer claims L2 is wired, `CLAUDE.md` no longer claims "LSP-backed", and `UNITS-TRACKER.md` no longer claims U-REPL is "fully landed (Stages 0–6)". |
| `docs/spec/current/drafts/decorators-dispatch-observability.md` D-1 (decision recorded 2026-07-13) | feature scope — deferred to v0.3 | A separate Dispatch-tier `@forwardMissing(to:)` decorator (forward *every* missed selector on a field via `doesNotUnderstand`) was considered and rejected for v0.2 in favor of Compile-tier `@delegate`'s explicit-selector-list-only surface. Revisit only if whole-interface delegation proves common enough in practice to earn a second decorator; until then the hand-written `Proxy`/DNU library ([proxy.md](../spec/current/drafts/proxy.md)) covers the open case. |
...
| `docs/spec/current/drafts/decorators-dispatch-observability.md` D-3 (decision recorded 2026-07-13) | feature scope — deferred to v0.3 | `@featureFlag` was specified against a ratified ambient `Flags` core module (global registry, `Flags.enabled(name)`), not an injected/per-scope `FeatureFlags` service. Revisit once dependency injection (`@inject`, [decorators-stdlib.md](../spec/current/drafts/decorators-stdlib.md)) is specified — the injected-service form is the natural upgrade path and does not require an `@featureFlag` grammar change, only a resolution-strategy change inside the decorator. |
...
| `docs/spec/current/object-model.md` §5:210-211; `docs/spec/.../implementation-status.md` | docs-drift | Note claims "every metaclass's superclass wired to `Class`, breaking it" — stale pre-U2; native tower now satisfies ADR-0002 rule 4 (tested) and U-INH extends the same rule to user classes. Re-point both. |
...
| `phalcom-core/src/compiler/lib.rs:~1081` (`has_new_construct` guard) + `expr.rs:103` (arity guard) | correctness — **DISSOLVED BY RULING 2026-07-15 (DEC-CTOR-H); Guard removed in U-CTOR-4** | **This row's premise was rejected.** It treated a wrong-arity `Sub.new()` falling through to the bare allocator as a *bug*. [TDR-0051](../../../decisions/accepted/0051-constructors-are-ordinary-class-side-methods.md) §7 rules the opposite: **`new()` is an ordinary inherited method**, so `C.new()` on a class whose only constructor is `new(n)` returns an object with every field `None` — **specified behavior**, exactly as Smalltalk. The compile-time guard at `expr.rs:103` was **deleted by U-CTOR-4**. |
...
| `docs/spec/current/core/floor-census.md:653` (§7 audit-hook count) | docs-drift | Pre-existing, unrelated to this unit: the prose said "count = 113" while `tests/invariants.rs`'s own `BASELINE + NEW... ` sum was already 115 before U-ANNOT-CONTRACTS's `+2` (U-SCHED's `+2` landed without updating this line). Bumped straight to the correct **117** (115 + this unit's own `+2`) rather than first restoring the intermediate 115, since the live test is the source of truth and the intermediate value has no reader. |
...
[STATE.md](STATE.md)'s landing record or `docs/decisions/STATUS.md` and carry their evidence.
...
| 13 | **Reassignment of a *captured* immutable binding is not rejected.** An outer binding reached through an upvalue from inside a block: the compiler only enforces immutability for a current-function local and a module global. An inner-block `count = count + 1` over an outer immutable `count` compiles to `SetUpvalue` with **no diagnostic**. Extend the assignment path to walk enclosing function-states. **NB:** filed against ADR-0014's `let`(immutable)/`var`(mutable) spelling; [TDR-0052](../../../decisions/accepted/0052-let-const-bindings-and-field-mutability.md) supersedes that spelling (`let`=mutable, `const`=immutable) but keeps every rule, so the hole is unchanged — it is now a hole in `const`. Re-verify against U-BINDINGS when it lands. | `phalcom-core/src/compiler/lib.rs` (`Expr::Assignment`, upvalue branch) | ADR-0014 → ADR-0064 | med |
| 14 | The `if(opt)` truthiness compile check (`CompilerError::OptionTruthiness`) is literal-only: it catches `None` and `Some.new(...)` as a condition, but not an Option-typed *variable* (that stays a runtime type error via the branch opcode's `Bool` requirement). No span attached. **This is the knowingly-accepted gap in [TDR-0019](../../../decisions/accepted/0019-no-truthiness-enforcement.md), not a bug** — closing it needs flow analysis Phalcom does not have. Keep as a record. | `phalcom-core/src/compiler/lib.rs` (`is_option_literal`) | ADR-0007/0021 | low |
...
| 26 | ~~Pre-existing `cargo doc` warning: `option` `some_new` links to private `wrap_some`.~~ **RESOLVED (U-ERR pass)** — repointed off the intra-doc link, `cargo doc` clean. Dup of #33. | `phalcom-core/src/primitive/nil.rs:64` | docs guidelines | done |
...
| 29 | ~~Scope-taxonomy divergence between the forge scheduling docs and `docs/spec/core/` on U-STD's ownership.~~ **RESOLVED (2026-07-12)** — U-STD scope settled via Option (B); the roster reconciliation landed as PHASE2-INDEX §7 pointing at `docs/spec/current/core/README.md` as index of record rather than forking it. | `docs/forge/PHASE2-INDEX.md` §7 | — | done |
...
| Debt | Owning unit |
|---|---|
| `primitive/number.rs:~34` — type-error message hardcodes `"value"` | [U12](units/U12/u12.md) §3 |
