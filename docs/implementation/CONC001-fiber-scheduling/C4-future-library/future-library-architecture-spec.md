# CONC001.C4 — Future library architecture spec
...
> **re-grounded against HEAD at activation** — the whole substrate ([Cooperative Fiber](../C1-fiber-execution/cooperative-fiber-architecture-spec.md))
...
> 1. **[Cooperative Fiber](../C1-fiber-execution/cooperative-fiber-architecture-spec.md) landed** — the `Fiber` substrate,
...
|---|---|---|
| **[Cooperative Fiber](../C1-fiber-execution/cooperative-fiber-architecture-spec.md)** | `Object::Fiber(FiberObject)`; `Fiber.new`/`call`/`try`/`yield`/`current`/`abort`; the O(1) switch + typed `ControlFlow`; the fiber-floor failure capture; **general `resumer` + result slot**. | **NOT landed** — v0.2 scope is bare `Fiber`, and even that is planned, not built. |
...
> ([Cooperative Fiber §3.2](../C1-fiber-execution/cooperative-fiber-spec.md#32-the-restriction-adr-0030-4)) applies:
...
| **Generator-specialized `Fiber`.** | Relies on [Cooperative Fiber](../C1-fiber-execution/cooperative-fiber-spec.md) keeping the `resumer` + result slot **general** (§6 seam); if Fiber specialized them, `await` breaks — verify at gate 1. |
