...
# CONC001.C2.P1 — ready queue and root drive
...
> "Proposed... unowned." [P2](../C4-future-library/P1-future-library-and-await.md) §9
...
## 2. Design summary (full detail: [`ready-queue-and-root-drive-spec.md`](ready-queue-and-root-drive-spec.md))
...
- **Does not require `main`-as-scheduler-fiber.** Supersedes
  ...
  scheduler fiber") with the root-drive pump `P2 §6.3`
...
- **Re-queuing a suspended (not finished) scheduled fiber is explicitly
  ...
  `P2 §10`'s must-not-preclude row on this exact hazard.
...
| `../../../../spec/current/system.md` | flip `schedule(_)` status; document `nextScheduled`/`runScheduled` as a floor amendment |
| `../../../../spec/current/concurrency.md` | flip Slice B's "needs the native ready-queue" note to landed, citing this unit |
...
| DEC-FUT-SCHED ruled P3 is the owning plan, not P2 | `../C4-future-library/P1-future-library-and-await.md` §9 |
| Root-drive pump (not scheduler-fiber-as-main) is the committed design | `../C4-future-library/P1-future-library-and-await.md` §6.3 |
| `System.schedule(_)`/`sleep(_)` reserved seam; native-only rationale (no `.ph` class-side mutable state) | `system.md` §2/§3 |
| `Future`'s pump `.try()`s ready drivers | `../C4-future-library/P1-future-library-and-await.md` §6.3 |
