...
category: CONC
program: CONC001
checkpoint: CONC001.C3
kind: corrective
status: PROPOSED
completion: NOT_STARTED
verification: UNVERIFIED
depends_on: [CONC001.C1.P1]
follows: CONC001.C1.P1
supersedes: null
deferred_reason: null
---

# CONC001.C3.P1 — fiber reflection and terminal state
...
> **Provenance.** Spun out of the Future plan ([`../C4-future-library/P1-future-library-and-await.md`](../C4-future-library/P1-future-library-and-await.md)) §2 gap 1, which
...
- **Does not preclude U-SCHED / Future Slice B.** `Future`'s pump
  ...
  exactly the two reads Slice B's design (`../C4-future-library/P1-future-library-and-await.md` §6.3) already
  assumes, unchanged.
...
| `../C4-future-library/P1-future-library-and-await.md` | strike the Slice-B write-set rows this plan now owns; replace with a precondition note (§4 below has the exact edit) |
...
## 4. Companion edit to `../C4-future-library/P1-future-library-and-await.md` (apply alongside this plan)

Replace `../C4-future-library/P1-future-library-and-await.md` §5's Slice B write-set row:
...
```
| **Precondition, not this plan's write-set:** `Fiber#isDone`/`error` ship via **CONC001.C3.P1** (standalone, unblocked, no scheduler dependency) — Slice B depends on it landing but does not implement it. |
```

And update §7 build order step 4 ("Slice B.0 — Fiber-surface completion") to
read: "Slice B.0 — confirm **CONC001.C3.P1** has landed (external
...
- **`Future` Slice B / U-SCHED:** *served, not precluded* — ships exactly the
  two reads Slice B's design already assumes (§6.3 of `../C4-future-library/P1-future-library-and-await.md`),
  with zero coupling to how U-SCHED eventually lands.
- **U-GC:** not precluded — pure reads, no new heap state, no change to
  `FiberObject`'s shape (see [MEMM001.C1.P1](../../../memm/MEMM001-garbage-collection/C1-gc/P1-garbage-collection.md) §2 preconditions,
...
| Coupling to DEC-FUT-SCHED was incidental, not load-bearing | `../C4-future-library/P1-future-library-and-await.md` §2 gap 1, §5 Slice B write-set, §7 build order step 4 |
...
with the new total) · the `../C4-future-library/P1-future-library-and-await.md` companion edit applied (§4) ·
