# CONC001.C1 — Cooperative fiber architecture spec
...
> deepened [cooperative-fiber-spec.md](cooperative-fiber-spec.md) (which extends
...
> those are the Deferred [Future library](../C4-future-library/future-library-architecture-spec.md) track.
...
- **`Future`/`async`/`await`/`then`/scheduler/ready-queue/`System.sleep`** — the Deferred
  [Future library](../C4-future-library/future-library-architecture-spec.md) track. Keep the `resumer` link + result
...
- **`iterate`/`for` machinery** — [LANG001 iteration protocol](../../../lang/LANG001-language-surface/C3-language-runtime/P7-iteration-protocol-and-for-lowering.md).
...
| **C-FIB-6** *(cross-unit, PENDING → graduates)* | [LANG001 iteration protocol](../../../lang/LANG001-language-surface/C3-language-runtime/P7-iteration-protocol-and-for-lowering.md)'s `pending/for_generator_suspends`: `Fiber.new { for (x in [1,2,3]) { Fiber.yield(x) } }` suspends, yields `1,2,3`. Flip to PASS once **both** land. | pending golden |
...
| **`Future`/`await` (Deferred [Future library](../C4-future-library/future-library-architecture-spec.md)).** | The `resumer` link + result slot stay **general** (§2.1 spec) — `await` suspends through exactly them. Not generator-specialized. |
...
| Surface (`new`/`call`/`try`/`yield`/`current`/`abort`), state machine | [concurrency.md](../../../spec/current/concurrency.md) §1; [cooperative-fiber-spec.md](cooperative-fiber-spec.md) §1–§2 |
...
| `for`-generator seam | [LANG001 iteration protocol](../../../lang/LANG001-language-surface/C3-language-runtime/P7-iteration-protocol-and-for-lowering.md); [TDR-0029](../../../decisions/accepted/0029-iteration-protocol-cursor.md) §5 |
