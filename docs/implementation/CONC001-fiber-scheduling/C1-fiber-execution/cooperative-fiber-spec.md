# CONC001.C1 — Cooperative fiber spec
...
> `for`-generator seam with the [LANG001 iteration protocol](../../../lang/LANG001-language-surface/C3-language-runtime/P7-iteration-protocol-and-for-lowering.md); the
> `yield`/resumer/result-slot seam with the later [Future library](../C4-future-library/future-library-spec.md)),
...
> `System` hooks** (those are the [Future library](../C4-future-library/future-library-spec.md), a **Deferred**
...
> the [Future library](../C4-future-library/future-library-spec.md) layer suspends `await` through *exactly*
> these two fields (`await` = "add `current` to the future's waiters, then
> `Fiber.yield`"). This is the [Future library](../C4-future-library/future-library-spec.md#the-fiber-seam)
...
| | Shape | Result |
|---|---|---|
| **✅ Suspends freely** | pure sends + **inlined** control flow (`while`/`ifTrue:` → `Jump`/`Loop`, one chunk) — the `counter` generator (§1.2); and, once the [LANG001 iteration protocol](../../../lang/LANG001-language-surface/C3-language-runtime/P7-iteration-protocol-and-for-lowering.md) lands, `for (x in coll) { Fiber.yield(x) }` | yield reaches the top loop |
...
[LANG001 iteration protocol](../../../lang/LANG001-language-surface/C3-language-runtime/P7-iteration-protocol-and-for-lowering.md)'s `for` (ADR-0035), which lowers to an inlined
...
### 7.1 `Fiber` ⊗ `for` — the generator seam (with [LANG001 iteration protocol](../../../lang/LANG001-language-surface/C3-language-runtime/P7-iteration-protocol-and-for-lowering.md))

*Cross-links [LANG001 iteration protocol](../../../lang/LANG001-language-surface/C3-language-runtime/P7-iteration-protocol-and-for-lowering.md),
...
land ([LANG001 iteration protocol](../../../lang/LANG001-language-surface/C3-language-runtime/P7-iteration-protocol-and-for-lowering.md),
§9 C-FIB-6 here).

### 7.2 `Fiber` ⊗ `Future` — the resumer/result-slot seam (with [Future library](../C4-future-library/future-library-spec.md)) {#future-seam}

*Cross-links [Future library §The Fiber seam](../C4-future-library/future-library-spec.md#the-fiber-seam);
[`concurrency.md`](../../../spec/current/concurrency.md) §2; ADR-0030 §1.*

`Future`/`async`/`await` are a **Deferred post-v0.2 track** ([Future library](../C4-future-library/future-library-spec.md)),
...
| **C-FIB-6** | *(cross-unit, PENDING → graduates)* [LANG001 iteration protocol](../../../lang/LANG001-language-surface/C3-language-runtime/P7-iteration-protocol-and-for-lowering.md)'s `Fiber.new { for (x in [1,2,3]) { Fiber.yield(x) } }` suspends and yields `1,2,3`. | pending golden |
...
- **`Future` / `async` / `await` / scheduler / `System` hooks** — the **Deferred**
  post-v0.2 [Future library](../C4-future-library/future-library-spec.md) track. U-FIBER ships **none** of it,
