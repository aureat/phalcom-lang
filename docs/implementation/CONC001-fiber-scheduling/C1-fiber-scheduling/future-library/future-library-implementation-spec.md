...
> [[U-FIBER]](../../../../forge/units/U-FIBER/implementation-spec.md) and adds **no new VM mechanism** beyond
...
> **re-grounded against HEAD at activation** — the whole substrate ([[U-FIBER]](../../../../forge/units/U-FIBER/implementation-spec.md))
...
> 1. **[[U-FIBER]](../../../../forge/units/U-FIBER/implementation-spec.md) landed** — the `Fiber` substrate,
...
|---|---|---|
| **[[U-FIBER]](../../../../forge/units/U-FIBER/implementation-spec.md)** | `Object::Fiber(FiberObject)`; `Fiber.new`/`call`/`try`/`yield`/`current`/`abort`; the O(1) switch + typed `ControlFlow`; the fiber-floor failure capture; **general `resumer` + result slot**. | **NOT landed** — v0.2 scope is bare `Fiber`, and even that is planned, not built. |
...
- **The `Fiber` seam** — `FiberObject.resumer` (dynamic caller chain) + the result slot,
  kept general by [[U-FIBER]](../../../../forge/units/U-FIBER/implementation-spec.md) (its §2.1). `await`
...
> ([[U-FIBER §3.2]](../../../../forge/units/U-FIBER/specification.md#32-the-restriction-adr-0030-4)) applies:
...
| **Generator-specialized `Fiber`.** | Relies on [[U-FIBER]](../../../../forge/units/U-FIBER/specification.md) keeping the `resumer` + result slot **general** (§6 seam); if U-FIBER specialized them, `await` breaks — verify at gate 1. |
...
| The `Fiber` seam (resumer + result slot; `await` = waiters + `Fiber.yield`) | [[U-FIBER §7.2]](../../../../forge/units/U-FIBER/specification.md#72-fiber--future--the-resumerresult-slot-seam); [concurrency.md](../../../../spec/current/concurrency.md) §2 |
