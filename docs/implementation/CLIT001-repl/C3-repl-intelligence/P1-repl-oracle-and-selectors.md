...
---
# CLIT001.C3.P1 — repl oracle and selectors

**Phase B — branch-only.** `phalcom-repl/**` only. `phalcom-lsp` is consumed as a
library and **never modified**.

This stage builds no user-visible surface. It builds the data [§06](P2-repl-completion-and-highlighting.md)
...
The reason is TDR-0054 ruling 6. Cells *shadow*: a later `class Foo` binds a new
class. Replaying all cells as one synthetic document would present two `class Foo`
declarations to the index — and under TDR-0055 that is `class.already_defined`, a
...
All testable without a terminal — this is why the stage is separate from [§06](P2-repl-completion-and-highlighting.md).
...
Two new modules rather than growing `completer.rs`: [§06](P2-repl-completion-and-highlighting.md) rewrites that
...
scope has leaked from [§06](P2-repl-completion-and-highlighting.md).
