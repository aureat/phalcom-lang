# CLIT001.C2 — Repl execution spec

Companion to [`../P1-repl-architecture.md`](../C1-repl-foundation/P1-repl-architecture.md) (§D-series, evaluation substrate) and
[`../repl-completion-and-highlighting-spec.md`](../C3-repl-intelligence/repl-completion-and-highlighting-spec.md) (§S-series, what the user sees). Those two say **what**
...
it is [§00](../C1-repl-foundation/P2-repl-branch-integration.md), and it changes the stage order.

## Status — three stages already landed

| Stage | Spec | Commit | State |
|---|---|---|---|
| §S8 — delete the rustyline stack | repl-completion-and-highlighting-spec.md §S8 | `380461c` | **landed** |
| §D2 — source binds to the artifact | P1-repl-architecture.md §D2 | `16b3760` | **landed** |
| §D7 parser half — EOF → `UnrecognizedEof` | P1-repl-architecture.md §D7 | `2fe6aba` | **landed** |
...
§D2's outcome, because [§02](P1-repl-session-and-cells.md) builds directly on `Chunk.source_id`.
...
Neither P1-repl-architecture.md nor repl-completion-and-highlighting-spec.md states this. §D1, §D3, §D8, §D9, §S1, §S2, §S3, §S5-L1
and §S5-L2 are all unreachable until it changes. That is why [§01](../C1-repl-foundation/P3-repl-compiler-wiring.md) exists
...
one. [§02 §2.2](P1-repl-session-and-cells.md) specs a separate, orthogonal type and keeps
`CompileMode` untouched. The name in P1-repl-architecture.md §D3 is the casualty; the design is not.
...
uncallable from there. [§02 §4](P1-repl-session-and-cells.md) specs the public surface.

**4. repl-completion-and-highlighting-spec.md's concurrent-edit note is stale.** It warns that another session held an
...
Ordered for **branch isolation**, which is not P1-repl-architecture.md's stage order. [§00](../C1-repl-foundation/P2-repl-branch-integration.md)
...
|---|---|---|---|---|
| 0 | [§01](../C1-repl-foundation/P3-repl-compiler-wiring.md) | crate deps; public VM surface | `phalcom-repl/Cargo.toml`, `phalcom-core` (3 visibility changes) | **yes — land on `main` first** |
| 1 | [§02](P1-repl-session-and-cells.md) | §D1 + §D10 + §D3 — session module, unwinding cell loop, echo | `phalcom-core` (compiler), `phalcom-repl` | **yes — split; see §00** |
| 2 | [§03](P2-repl-cell-immutability.md) | §D4 — two-set immutability, cross-cell rebind | `phalcom-core` (compiler) | **yes — split; see §00** |
| 3 | [§04](P3-repl-continuation.md) | §D7 REPL half — validator, `\`, `...`, escapes | `phalcom-repl` only | no |
| 4 | [§05](../C3-repl-intelligence/P1-repl-oracle-and-selectors.md) | §D8 + §D9 — live oracle, structured selectors | `phalcom-repl` only | no |
| 5 | [§06](../C3-repl-intelligence/P2-repl-completion-and-highlighting.md) | §S1–§S7 — completion, hints, highlighting, latency | `phalcom-repl` only | no |
| 6 | [§07](../C4-repl-operations/P1-repl-commands.md) | §S9 — `:reload` | `phalcom-repl` only | no |
| — | [§08](../C4-repl-operations/P2-repl-consolidation.md) | merge protocol, test matrix, exit criteria | — | — |
...
- **The §D3 type is new and orthogonal, not a `CompileMode` variant.** Ruled in
  [§02 §2.2](P1-repl-session-and-cells.md). Rationale is delta 2 above.
- **`unwind_to` is exposed via a named `VM` method, not by making the existing one
  `pub`.** Ruled in [§02 §4](P1-repl-session-and-cells.md).
