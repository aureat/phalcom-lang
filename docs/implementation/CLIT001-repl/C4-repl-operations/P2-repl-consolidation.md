...
---
# CLIT001.C4.P2 — repl consolidation
...
- [ ] `../P1-repl-architecture.md` §D3's `CompileMode::Repl` wording corrected to `UnitKind`
      ([README delta 2](../C2-repl-session-execution/repl-execution-spec.md)), or annotated as superseded by this directory.
- [ ] `../repl-completion-and-highlighting-spec.md`'s stale concurrent-edit note removed ([README delta 4](../C2-repl-session-execution/repl-execution-spec.md)).
...
|---|---|---|
| `open_upvalue_hygiene_across_cells` | [§02](../C2-repl-session-execution/P1-repl-session-and-cells.md) | `unwind_cell()` replaced by a raw `clear()` |
| `const_redeclares_across_repl_cells` | [§03](../C2-repl-session-execution/P2-repl-cell-immutability.md) | exemption that relaxes only the const check |
| `const_assignment_errors_in_file_mode` | [§03](../C2-repl-session-execution/P2-repl-cell-immutability.md) | exemption leaking into `File` units |
| `validator_matches_probe_classification` | [§04](../C2-repl-session-execution/P3-repl-continuation.md) | validator drifting from the parser's rule |
| `value_echo_survives_raising_tostring` | [§06](../C3-repl-intelligence/P2-repl-completion-and-highlighting.md) | echo turning a good cell into a failed one |
| `reload_survives_declarations` | [§07](P1-repl-commands.md) | `:reload` reusing the session `Compiler` |

## 2. Merge order

**If Phase A landed on `main` as planned** ([§00](../C1-repl-foundation/P2-repl-branch-integration.md)), the branch
...
| `phalcom-ast/src/parser.rs` | CLASSCLOSE's nested-class rejection `:1557-1559`; U-REPL's optional shared `classify` | Keep both. If `classify` was not extracted ([§04 §6](../C2-repl-session-execution/P3-repl-continuation.md)), no conflict. |
...
**This is flagged, and it is already handled.** [§02 §1.4](../C2-repl-session-execution/P1-repl-session-and-cells.md)
writes the cell loop against TDR-0054 ruling 6 (cells shadow, never reopen) — the
post-CLASSCLOSE world — from the start. Whichever unit lands second, U-REPL's behavior
is identical and nothing rebases.

Confirm at merge with `class_redefinition_shadows` ([§02 §4](../C2-repl-session-execution/P1-repl-session-and-cells.md)):
...
Carried forward from P1-repl-architecture.md. Check each at consolidation:
...
- This directory's [README](../C2-repl-session-execution/repl-execution-spec.md) status table — mark the remaining stages landed
