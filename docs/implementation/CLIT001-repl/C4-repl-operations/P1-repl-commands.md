...
Dispatch commands **before** the validator ([§04](../C2-repl-session-execution/P3-repl-continuation.md)) sees the line —
...
Input is `ReplSession.history` ([§02 §1.2](../C2-repl-session-execution/P1-repl-session-and-cells.md)), in submission
order.
...
1. §D4's two-set immutability ([§03](../C2-repl-session-execution/P2-repl-cell-immutability.md)),
2. U-BINDINGS' same-scope redeclaration ban (`compiler/lib/scope.rs:118`, `:170`),
3. TDR-0055's registration of class declarations in the same `global_bindings` map.
...
cross-cell regression test in [§03](../C2-repl-session-execution/P2-repl-cell-immutability.md) guards, and the same reason that
test must not be weakened.

Since each cell already compiles through its own `Compiler` ([§02](../C2-repl-session-execution/P1-repl-session-and-cells.md)),
