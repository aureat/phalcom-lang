...
---
# CLIT001.C1.P3 — repl compiler wiring

**Phase A. Lands on `main`.** Prerequisite for every later stage.

Neither P1-repl-architecture.md nor repl-completion-and-highlighting-spec.md states this stage, because both assume the REPL crate can
...
- Does not call the VM from `repl.rs`. `eval` stays a stub until [§02](../C2-repl-session-execution/P1-repl-session-and-cells.md).
- Does not add the unit-kind type (§D3). That is [§02 §2.2](../C2-repl-session-execution/P1-repl-session-and-cells.md).
