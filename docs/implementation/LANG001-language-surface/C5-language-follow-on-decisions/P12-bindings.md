...
# LANG001.C5.P12 — bindings

Companion to [`err-plan.md`](u30-bindings-plan.md). Governed by
...
Every precondition in `err-plan.md` was re-verified against the tree at `de49d3a` on 2026-07-19
...
Five `err-plan.md` claims did not survive that verification. They are corrected in
...
`err-plan.md` says "verify on HEAD — do not trust this list". Done. Results:
...
| P4 | `let` on a field is unenforced | ✅ **reproduced** | the `clobber` probe prints `99`, no error, exactly as `err-plan.md` predicts |
...
`err-plan.md` calls P4 and P7 load-bearing. Both hold, so §3 of the plan is neither
already-done nor under-specified. **But P4's motivating defect has zero live instances**
— see §1.5.

---

## 1. Corrections to `err-plan.md` — read this first

### 1.1 There is no `FIELD` token, so the plan's disambiguation argument is void

`err-plan.md` §3 justifies the bare-`_x` declaration form like this:
...
`err-plan.md` lists as a hard gate: "**byte-identical golden stdout across the codemod**".
...
`err-plan.md` states "The risk here is entirely in the **1080-site codemod**" and recommends an
...
`err-plan.md`'s write-set lists the spec grammar, keyword lists, `classes.md` §2, and the
...
| `docs/decisions/accepted/0014-…:3` | already `Superseded by ADR-0064` |
| `docs/decisions/STATUS.md:44` | already `Superseded by 0064` |
...
2. `statements-and-declarations.md:81` writes the field production as
   ...
   almost certainly the origin of `err-plan.md`'s false "fields lex as `FIELD`" claim** (§1.1) —
...
`docs/decisions/README.md:57` still lists ADR-0014 as **Accepted**, while the ADR file itself and
...
`err-plan.md` §Role motivates the unit partly on the unenforced-`let`-field hole. The hole is
...
- Attributes precede the optional `const`, so `@get @set _x` and `@class const _limit = 10`
  both parse with no further grammar change. `err-plan.md` §"not preclude" asks for the second
...
| `docs/decisions/accepted/0014-*.md`, `docs/decisions/STATUS.md` | ⚠️ **already flipped (§1.5) — no action** |
| `docs/decisions/README.md:57` | ADR-0014 row still says Accepted; **ADR-0064 has no row at all** (§1.6) |
...
`err-plan.md`'s order leaves the tree red in the middle: step 1 deletes `Token::Var`, but the
...
Not "docs" as `err-plan.md` framed it — the syntax spec and the ADR-0014 flip are already done
...
- **`docs/decisions/README.md:57`** (§1.6).
...
same day, once [TDR-0053](../../../decisions/accepted/0053-classes-are-closed.md) (*Classes are
...
1. **ADR-0061's ratification is not yet filed, deliberately.** The ruling is recorded here
   and in the banner, but the ADR file still sits in `docs/decisions/proposed/` marked Proposed.
   The edit was held because the repo is mid-convention-change:
   [`docs/decisions/README.md`](../../../pdr/README.md) (uncommitted, authored by a
   ...
   it also means editing `docs/decisions/` structure while another session works there
   uncommitted. **The minimal non-colliding action** — flip the status line in place, update
   `docs/decisions/STATUS.md`, fix `docs/decisions/README.md`'s rows (0056/0059 point at `proposed/`;
