...
[`U-CLASSNS`](../U-CLASSNS/u31-classns-spec.md)**: the redefinition error is undecidable until
class identity is `(module, name)`-keyed.

`err-plan.md`'s preconditions were re-verified 2026-07-19 against the tree. **Two of its load-bearing
...
for compile errors to hang a second span on (§1.2). A third — the `@variant` hazard `err-plan.md`
...
`err-plan.md` §3.3 and PDR-0001 ruling 4 both reason from "`Bytecode::Class(u16)` means two
...
**This is a decision, not a fait accompli** — see §14.1. `err-plan.md` and the dispatch handoff both
...
`err-plan.md` §3.4 and its rubric both call `@variant` "the single most likely way to break the
...
`err-plan.md`'s "test `@variant` expansion explicitly, first" stands — not because the ban might trip
...
`err-plan.md` P6 says "`Statement::Class` is position-unrestricted — the parser imposes nothing." The
...
`err-plan.md` §7 and the 2026-07-19 ruling say the two IC fixtures become Rust-level tests that
...
| What | `err-plan.md` / 0065 says | Actual |
|---|---|---|
