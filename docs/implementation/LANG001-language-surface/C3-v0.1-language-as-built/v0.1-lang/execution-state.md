...
## U-CORE-1 — LANDED ✅ (2026-07-12, `03764e3`, in-tree on `main`, no worktree)
- **Scope:** kernel reflection, per [`U-CORE-1-implementation-spec.md`](../../../spec/core/U-CORE-1-implementation-spec.md)
  (re-grounded against HEAD `de03f26` before dispatch, commit `010268c`). First unit of the
  `docs/spec/core/` successor track (§0.4 of the spec — subsumes the coarser U-STD/U11 "reflection"
  planning). **7 new floor primitives** (ADR-0023, floor **73 → 80**): `Object#hash` (identity
  digest of the heap handle) + value-based `hash` overrides on `Number`/`String`/`Symbol`/`Bool`;
  `Behavior#name` (shadows `Object#name`'s metaclass-name result for class receivers) and
  `Behavior#methods` (own method-dictionary selectors). `Object#isA(_)` added to `core.ph` as pure
  `.ph` — derivable over `class`/`==`/`superclass`, no floor addition. `Method < Function` re-parent
  + load-order fix (decisions.md §4.1, ADR-0006) executed in `create_core_classes`.
- **Invariant substrate stood up (R-INV-0.1…0.4)** — every later U-CORE unit extends this: a
  live-`VM::new()` census audit asserting the exact 80-binding `(class, selector)` set against
  `floor-census.md`; the `verify_invariants` parallel-rule check extended from `Number`-only to all
  17 ordinary tower rows (incl. `True`/`False`, which carry zero own bindings); absence
  non-surfacing and fixed-slot layout (`Some`=1, `Message`=4) boot checks. Plus R-INV-1.2–1.6 unit
  invariants (`isA` reflexivity, `hash`↔`==` consistency, `hash` stability, `Method.superclass ==
  Function`, reflection side-effect-freedom).
- **Fixtures:** `metaclass/pending/metaclass_is_a` flipped to active (`pending/` now empty) +
  5 new goldens (`metaclass_is_a_object_root`, `reflection/{hash_stable,hash_distinct,
...
- **ADR-0023** (omnibus ADR-0019 floor amendment covering this unit plus U-CORE-3/4/6's amendments)
  was ratified **Accepted** ahead of dispatch — see `docs/decisions/accepted/0023-amend-floor-admit-hash-and-kernel-reflection.md`.
...
Independent reviewer ON: **U1, U2, U3, U4, U6** (heap, metaclass tower, dispatch, blocks, absence — can corrupt the object model). Reviewer OFF (self-verify on green gate + `cargo doc`): U5, U7, U8, U-LEX, U-STD, U9, U10, U11, **U-CORE-1**. All units still gate on `./scripts/verify.sh` green + docs.
