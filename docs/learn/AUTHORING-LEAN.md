...
| Claims labelled unverified rather than smuggled | 3 | 3 | 3 | **2** — that the `.attempt()` wrapper was chosen to *detect* the root case (the code's shape says so; no record does); DEC-FUT-SCHED having no `docs/pdr/` entry, so the ruling lives only in `err-plan.md` §9 |
