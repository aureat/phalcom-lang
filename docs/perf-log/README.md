...
| [010](010-prereg-h16-h17-h13.md) | H16 / H17 / H13 | **NOT MEASURED — built and pre-registered only.** Six timing binaries at a single common base (`4c6c83f`), each verified behavior-invariant, waiting on a quiet box. Retires H17's prebuilt `e66af34` pair: `4c6c83f` edited **inside `run_until_inner`** and shrank it 248 B, so a stale base cannot anchor a ~5% effect. Verified without timing — H16's probe cuts the loop **−4,208 B (−25.8%)** leaving `call_method`/`invoke_at` byte-identical; **H17 leaves `run_until_inner` at +0 B, so F21's tax provably cannot apply to it**; F23's counts reproduce at HEAD (**−6,000,001**, **−2,000,001 `Return`**) across the `raw*` rename; H13's obvious probe was **confounded** (+96 B `call_method`, memcpy→loop) and replaced by a clean one (−24 B) | *(no wall-clock exists — see [§7](010-prereg-h16-h17-h13.md#7-the-quiet-window--one-run-six-arms) for the run)* | n/a | *(uncommitted)* |
...
4. **U-HOTPATH Change 1 — hoist the *frame* out of the dispatch loop.** Not optional
   ...
   **Still open on this item:** S1b (hoist `ip`/`stack_offset` — needs a
   frame-identity guard, *not* 007's `closure_id` guard, which two fibers in one
   closure defeat), S3 (safepoint on back-edges only), S4 (`CallFrame` 96 → ~32 B).
