...
- [x] **U-SEQ** — [plan](units/U-SEQ/plan.md) · [spec](units/U-SEQ/implementation-spec.md) — historical U-SEQ record retained: `606829e` combinator breadth + lazy view classes (steps 1–2), `e6e767a` sugar wiring (step 3, **DEC-SEQ-A = Branch A**), `838244a` lazy-semantics comments. **Current D.1/E status:** direct collection transforms are eager, predicate queries use labeled `where:`, Map pair traversal uses `map.entries.each`, and laziness starts at explicit `.iter` pipelines. Focused sequence and D.1 lanes are green; deferred spread fixtures remain separate pending work. **Open:** no `as-built.md`; spec §5's `all_generator_raises.ph` fiber fixture was never written (only `concurrency/each_generator_raises.ph` exists).
