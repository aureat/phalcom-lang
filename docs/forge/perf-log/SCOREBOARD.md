...
> **H16, H17 and H13's body half are BUILT AND PRE-REGISTERED, awaiting a quiet
> box — see [010](010-prereg-h16-h17-h13.md).** Six timing binaries at a single
> common base (`4c6c83f`), behavior-invariance verified, predictions recorded
> before any number exists; [§7](010-prereg-h16-h17-h13.md#7-the-quiet-window--one-run-six-arms)
> has the exact commands. Two of the rows below are already amended by facts
> established there **without timing**: **H17's prebuilt `e66af34` pair is
> retired** (`4c6c83f` edited inside `run_until_inner` and shrank it 248 B, so
> that base is stale — rebuild, do not reuse it), and **H17's "immune to F21"
> claim is now measured, not argued** (`run_until_inner` is byte-identical at
> +0 B). F23's counts also reproduce at HEAD unchanged by the `raw*` rename.
