...
| *(this commit)* | **Cut 009 — superinstructions: fuse `GetSelf -> GetField`** — **NEGATIVE, reverted, nothing landed** ([009](009-fuse-getself-getfield.md), [F21](findings.md#f21--an-arms-code-is-paid-by-every-program-not-the-ones-that-execute-it)) | `method_call` **−2.2%** for exactly the **−6,333,335** dispatches predicted. But **`string_equals` +5.9%**, **`fib` +4.8%**, **`for` +4.4%** — rows retiring **identical** instruction counts and executing **zero** `GetSelfField`. Probes: the 38th variant with an *empty* arm is **free** (−0.6%); a *real* arm that is **never emitted** costs **+6.1%**. The arm's **code**, not the fusion, not the variant count | unchanged | none |
