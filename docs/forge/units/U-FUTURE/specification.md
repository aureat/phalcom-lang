...
> [[U-FIBER]](../U-FIBER/specification.md) landing** — `Future` derives entirely from
...
>   [[U-FIBER]](../U-FIBER/specification.md) on HEAD; confirm the `resumer` link + result
...
exactly three things, **all of which depend on [[U-FIBER]](../U-FIBER/specification.md)**:
...
sequenced **after [[U-FIBER]](../U-FIBER/specification.md), before any `Future` method
...
*Cross-links [[U-FIBER §7.2]](../U-FIBER/specification.md#future-seam).*

`Future` layers over `Fiber` at exactly two [[U-FIBER]](../U-FIBER/specification.md)
...
`block_call` raises `CannotYieldAcrossNativeFrame` ([[U-FIBER §3.2]](../U-FIBER/specification.md#restricted-yield)).
