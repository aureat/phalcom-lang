...
- **New `Job`/`Payload` variants** (plain data, structural boundary):
  ...
  `"timedOut"`, `"dnsFailure"`, `"closed"` — stdlib/net.md §9 law 3's `Err` kinds).
...
### 2.4 Close-with-pending (stdlib/net.md §7)
...
| `Dns.resolve_(_,_)` static | `.ph` `Dns.resolve(_)` | host-`Bytes` (the `String`'s UTF-8), future; `Job::DnsResolve` on the **worker pool** — std `(host, 0).to_socket_addrs()` is the blocking `getaddrinfo`; dedupe-preserving-order to `AddrList`. Empty ⇒ `IoErrorData("dnsFailure")` (stdlib/net.md §6) |
...
stdlib/net.md §10's harness row-for-row in golden lanes `net/` + `net/negative/`
...
| echo | stdlib/net.md §10's two-fiber loopback echo through the real poller |
...
| accept FIFO / port 0 / refused / EOF-on-FIN / shutdown rows | stdlib/net.md §10 verbatim |
...
TLS (own record; obligations in stdlib/net.md §8), UDP (Q-N1), Unix sockets (Q-N2), IDNA
