...
[U-FUTURE's DEC-FUT-SCHED ruling](../U-FUTURE/plan.md#9-blocked-on-decision-register):

| Unit | Mission | Dispatch status | Depends on |
|---|---|---|---|
| [U-FIBER-REFLECT](U-FIBER-REFLECT/plan.md) ([implementation-spec](U-FIBER-REFLECT/implementation-spec.md)) | `Fiber#isDone`/`Fiber#error` — pure reads, no scheduler dependency | **dispatch-ready, no blockers** | landed `Fiber` (U-FIBER) only |
...
because both exist for the same reason: [U-FUTURE/plan.md §9](../U-FUTURE/plan.md#9-blocked-on-decision-register)
...
`docs/forge/units/U-FUTURE/plan.md` §7 build order steps 4–7) — not built
