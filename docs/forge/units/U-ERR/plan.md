...
**[error-handling.md](../../../spec/v0.2/error-handling.md)** §1–§6 + **[result.md](../../../spec/v0.2/result.md)** §1–§5.
...
> **Unit-name note.** This is the "later unit" that `docs/spec/v0.2/core/decisions.md` §Q2 and
...
|---|---|---|---|
| **DEC-ERR-A** ⚠ **BLOCKED-ON-ADR** | **The ADR-0019 amendment for `Block#on`/`ensure` (+2).** Not pre-cleared by ADR-0023. | **(A)** author a new per-unit landing-record ADR-0019 amendment (mirror ADR-0028), cite ADR-0008 as design authority, `+2 bindings`, census bump; **(B)** try to fold into an existing omnibus. | **(A)** — a fresh per-unit ADR; claim the next free number at dispatch (`ls docs/adr/`). This is a bookkeeping/landing ADR, not a design ADR (ADR-0008 already governs). The `documentation-and-adrs` skill drafts it. **This is the one gate that must clear before the step-2 diff merges.** |
...
## 8. Must-not-preclude check ([error-handling.md](../../../spec/v0.2/error-handling.md), [forward-compat.md](../../../spec/v0.2/core/forward-compat.md) §2/§7)
