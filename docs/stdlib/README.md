# `docs/stdlib/` — the standard library build program

- Status: **Program index.** Not ratified, not an ADR, not a normative spec.
- Date: 2026-07-20
- Scope ratified by the user on 2026-07-20; recorded in
  [`drafts/stdlib-catalog.md` §Amendment](../spec/v0.2/drafts/stdlib-catalog.md#amendment-2026-07-20--the-ratified-build-program).
...
[`docs/spec/v0.2/drafts/stdlib-catalog.md`](../spec/v0.2/drafts/stdlib-catalog.md) (Tier 0–6,
...
[`drafts/README.md`](../spec/v0.2/drafts/README.md) plus this one:

| Tier | Where | Means |
|---|---|---|
| Ratified decision | `docs/adr/accepted/` | Committed. Needs a superseding ADR to change. |
| Normative spec | `docs/spec/v0.2/*.md` | The designed surface. Cites its ADR. |
| As-built | `docs/forge/units/*/as-built.md` | What shipped, with `file:line`. |
| Draft | `docs/spec/v0.2/drafts/` | Exploration. No authority. |
| **Program** ← *you are here* | `docs/stdlib/` | **A selected, ordered work list.** Each item's spec file, once written, is a design document — it becomes normative only by growing an ADR. |
...
[`system.md`](../spec/v0.2/system.md), the scheduler is a bare ready-queue, and there is no
...
[`stdlib-catalog.md` §Open questions](../spec/v0.2/drafts/stdlib-catalog.md#open-questions).
...
| **S-7** does `System` remain the single effect receiver | 8, 9, 14 | [`system.md`](../spec/v0.2/system.md) §1's rule is "effects are named, not ambient". If `Fs`/`os` become plain globals, that property is lost in one commit. |
