...
- Deferred: 2026-07-20
- Raised by: [TDR-0057](../../decisions/accepted/0057-resources-are-disposable-handles-not-finalized.md) §3
- Blocks: nothing today. Becomes load-bearing the first time a generic IO algorithm exists.
- Related: [TDR-0041](../../decisions/accepted/0041-amend-iteration-bare-cursor-sentinel-and-iterable-root.md)
  (`Iterable` as a kernel root — the precedent 0069 §3 followed for one axis),
  [TDR-0010](../../decisions/accepted/0010-static-instance-slot-layout.md) (frozen slot offsets — the reason
...
TDR-0058 §3 resolved this for now by reifying **only** closeability (`Resource` as a kernel
...
- **It does not touch layout.** [TDR-0010](../../decisions/accepted/0010-static-instance-slot-layout.md)
...
- **It does not touch dispatch.** No new method sources, so
  [TDR-0011](../../decisions/accepted/0011-selector-signature-encoding-and-dispatch.md)'s lookup and the
...
declaration can require a full selector *signature* (arity + labels, TDR-0011's encoded form) or
only a bare name; and how this interacts with [TDR-0053](../../decisions/accepted/0053-classes-are-closed.md)'s
...
1. **Full mixins / traits.** Composable units carrying implementation and possibly state. Strictly
   more powerful and strictly more expensive — touches TDR-0010 layout and TDR-0011 dispatch as
