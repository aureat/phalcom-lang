...
1. **`Fiber#isDone` and `Fiber#error` are NOT landed.** `concurrency.md §1` lists them, but
   ...
   **no `.ph`-observable way to tell "the fiber I just resumed terminated" from "it yielded"**
   ...
   (2026-07-13): spun out to [U-FIBER-REFLECT](../reflect/plan.md)** — both are pure
...
| `err-plan.md` (+ status) | this plan | disjoint |
...
| File | Why |
|---|---|
| — | **Precondition, not this unit's write-set:** `Fiber#isDone`/`error` ship via **[U-FIBER-REFLECT](../reflect/plan.md)** (standalone, unblocked, no scheduler dependency) — Slice B depends on it landing but does not implement it. |
...
4. **Slice B.0 — confirm [U-FIBER-REFLECT](../reflect/plan.md) has landed** (external
...
| `Fiber#isDone`/`error` not landed → ships via standalone U-FIBER-REFLECT, a Slice B precondition | `universe.rs` L487–496 vs concurrency.md §1; `../reflect/plan.md` |
