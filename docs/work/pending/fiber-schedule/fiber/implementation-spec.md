...
| **`Future`/`await` (Deferred [[U-FUTURE]](../../../work/pending/fiber-schedule/future/implementation-spec.md)).** | The `resumer` link + result slot stay **general** (§2.1 spec) — `await` suspends through exactly them. Not generator-specialized. |
