...
| G8 | **FIFO is asserted but not specified.** `concurrency_sched_fifo_order` pins FIFO ordering. Scheduler fairness policy is **OPEN** (concurrency.md §3, TDR-0027). The test currently converts an open question into a de-facto contract by accident. | Nothing distinguishes contract tests from characterization tests. | **Lane E** §4 |
