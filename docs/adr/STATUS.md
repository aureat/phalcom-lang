...
> **This folder is being migrated to [`docs/decisions/`](../decisions/README.md).** New
...
| 0063 | Constructors are ordinary class-side methods — `@constructor`/`@class` decorators, `new_` allocator | **Accepted** (ratified 2026-07-15), **amended same day** | | ❌ **decision ratified, not built** — the status/reality gap here is the *inverse* of 0056/0061: the ADR is Accepted and the tree is entirely pre-decision. **Amended 2026-07-15 pre-implementation** (DEC-CTOR-F/G/H/I): `@static`+`@classField` → a single **`@class`** (supersedes the same ADR's own A/A1 ruling); **§7's tombstone and the compile-time arity guard are deleted** — `new()` is an ordinary inherited method, so this ADR **no longer closes `DEFERRED.md:29`, it dissolves it by ruling**; **§6.1 adds a `native_repr` category** (only `Object::Instance`-backed classes may bare-allocate — new machinery, `ClassObject` has no such flag today); **§10 adds the `class` keyword-variable** (dynamic ≡ `self.class`). Field grammar defers to [0064](accepted/0064-let-const-bindings-and-field-mutability.md). Implementation is [U-CTOR](../forge/units/U-CTOR/plan.md) (5 sub-units, **after U-BINDINGS**; U-CTOR-5 measurement-gated per DEC-CTOR-C, so the desugar is **not** ratified by this row) Current state is the *pre*-decision one: `construct`/`static` are keywords (`phalcom-ast/src/lexer.rs:284-285`), `ClassMember::Construct`/`ConstructDef` exist (`ast.rs:197,321`) with no `attributes` field, `parse_attribute` special-cases `Token::Construct` (`parser.rs:1046`), `SignatureKind::Initializer` is live (`class_decl.rs:643`), and the floor carries the byte-identical duplicate pair `class_new`/`object_class_new` (`primitive/class.rs:107`, `primitive/object.rs:105`) registered as `Class >> new()` + `Object class >> new()` (`universe/primitives.rs:47,100`). **Reverses `classes.md` §1's "no user-visible allocator" direction** — which was never built — and supplies the constructor-installation record that ADR-0011 was being miscited for. Floor amendment (ADR-0019): net **−1** fn/binding |
