...
- Status: Accepted
- Date: 2026-07-11
- Related: `docs/spec/blocks.md` §5; `docs/spec/object-model.md` §4; [ADR-0006](0006-function-as-abstract-callable-root.md); [ADR-0009](0009-handle-arena-heap.md)

## Context

Blocks are the keystone construct — a block, a lambda, a method body, and a getter
body share one closure representation ([Blocks](../spec/blocks.md), [ADR-0006](0006-function-as-abstract-callable-root.md)).
...
- **Escaping blocks.** A block may outlive the frame in which it was created
  ([Blocks §5](../spec/blocks.md)); captured variables must remain live and, where
...
- **Non-local return.** `return` inside a block unwinds to its *home method* frame,
  ...
  ([Blocks §5](../spec/blocks.md), [Object Model §4](../spec/object-model.md)).
...
- Escaping blocks and shared mutation of captured variables both work, matching
  [Blocks §5](../spec/blocks.md); an inner block that mutates a captured `var` is
...
- Non-local return is safe by construction: the generation check converts "return to
  ...
  form is expression-only and cannot carry `return` ([Blocks §2](../spec/blocks.md)),
  which keeps the safe cases the common cases.)
- One closure representation means `Fiber`/`Future` ([Concurrency](../spec/concurrency.md))
