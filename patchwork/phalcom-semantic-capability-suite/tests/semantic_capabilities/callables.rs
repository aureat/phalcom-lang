...
"#);
    ...
    let signature = surface
        .get_callable(DispatchSide::Class, &make_id.selector)
        .expect("Factory.make signature");
...
"#);
    ...
    assert!(g_call.knowledge.is_unknown() || g_call.knowledge.is_dynamic(), "recursive call must remain epistemically incomplete: {g_call:#?}");
    assert!(f_call.knowledge.is_unknown() || f_call.knowledge.is_dynamic(), "recursive call must remain epistemically incomplete: {f_call:#?}");
}
