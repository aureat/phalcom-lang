use crate::support::{assert_refuted, assert_validated, Fixture};
...
fn tuple_supertype_annotation_preserves_specific_product_fact() {
    let f = Fixture::new(r#"
class Option {}
class Some is Option { @constructor new() {} }
class Probe {
  @class
  run() {
    let x: (Number, Option) = (1, Some.new())
  }
}
"#);
    let number = f.ty("Number");
    let option = f.ty("Option");
    let int_ty = f.ty("Int");
    let some = f.ty("Some");
    let run = f.callable("Probe", "run", DispatchSide::Class);
...
class Probe {
  @class
  run() {
    let x: (Number, Option) = ("bad", Some.new())
  }
}
"#);
    let number = f.ty("Number");
...
class Probe {
  @class
  run() {
    let pair: (Number, Option) = (1, Some.new())
    let (x, y) = pair
  }
}
"#);
