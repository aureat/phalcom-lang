...
description: >-
  Implementation guide for extending Phalcom semantic analysis in Rust. Use
  when adding or modifying scope/name resolution, semantic occurrences,
  receiver/value inference, dispatch, field/parameter facts, structured flow
  or CFG analysis, callable summaries, interprocedural fixed points, module
  dependency/invalidation, semantic queries, LSP-backed facts, type-checker
  hooks, proof/effect analyses, or tests/performance instrumentation. Builds on
  Phalcom's existing phalcom-lsp semantic database and preserves a migration
  path toward shared compiler/checker/static-prover semantics.
...
Use that skill for semantic doctrine; use this one to change code safely.
