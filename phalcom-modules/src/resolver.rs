...
/// Module resolver coordinating ProjectUniverse root tables with one
/// ownership-authoritative SourceProvider.
...
impl<'u, P: SourceProvider + ?Sized> ModuleResolver<'u, P> {
    ...
    /// Resolves an AST ImportPath written inside `importer`. Resolution errors,
    /// package-interface errors, and parse errors retain their typed payloads.
        ...
        match &syntax.root {
            ImportRoot::Absolute(root_seg) => {
                ...
                for seg in &syntax.segments {
                    components.push(
                        ModuleComponent::from_identifier(&seg.name)
    ...
    fn absolute_root_target(
        ...
        match root.as_str() {
            "universe" | "core" => {
                return Ok((ProjectIdentity::Builtin(BuiltinProject::Universe), "universe".to_string(), true));
            }
    ...
    /// Validates hierarchical exposure for an external persistent or builtin
    /// project without erasing package parse/interface errors.
    ...
    /// Loads and parses the unlinked interface of a module. Cache entries are
    /// generation-scoped so LSP/project refreshes cannot retain stale mappings.
    ...
    fn load_interface_uncached(&self, module_id: &ModuleId) -> Result<UnlinkedModuleInterface, ModuleLoadError> {
        ...
        InterfaceBuilder::build(module_id.clone(), unit.kind, &parse_result.program).map_err(|error| ModuleLoadError::Interface {
            module: module_id.clone(),
            error,
        })
    }
