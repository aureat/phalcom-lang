...
use crate::interface::{
    DeclarationSurface, ExportSurface, InterfaceBuilder, UnlinkedExportTarget,
...
static BUILTIN_PARSED_CACHE: OnceLock<
    Mutex<HashMap<ModuleId, Result<Arc<ParsedModuleUnit>, ModuleLoadError>>>,
> = OnceLock::new();
static BUILTIN_INTERFACE_CACHE: OnceLock<
    Mutex<HashMap<ModuleId, Result<UnlinkedModuleInterface, ModuleLoadError>>>,
> = OnceLock::new();

fn parsed_cache(
) -> &'static Mutex<HashMap<ModuleId, Result<Arc<ParsedModuleUnit>, ModuleLoadError>>> {
    BUILTIN_PARSED_CACHE.get_or_init(|| Mutex::new(HashMap::new()))
}

fn interface_cache(
) -> &'static Mutex<HashMap<ModuleId, Result<UnlinkedModuleInterface, ModuleLoadError>>> {
...
impl BuiltinInterfaceBuilder {
    ...
    pub fn build_from_parsed(
        _provider: &BuiltinProjectSourceProvider,
        parsed: &ParsedModuleUnit,
    ) -> Result<UnlinkedModuleInterface, ModuleLoadError> {
        let mut iface = InterfaceBuilder::build(parsed.id.clone(), parsed.kind, &parsed.program)
            .map_err(|e| ModuleLoadError::Interface {
        ...
        if id.project == ProjectIdentity::Builtin(BuiltinProject::Universe) {
            let names: Vec<&'static str> = if id.path.is_root() {
                phalcom_native_meta::UNIVERSE_BINDINGS
                    .iter()
                    .filter(|b| b.exported)
                    .map(|b| b.name)
                    .collect()
