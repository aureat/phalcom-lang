// Ported from wren/test/benchmark/map_string.wren. `map[key] = i` /
// `map[key]` index-assign/read replaced with the documented Map surface
// `.at(k, put: v)` / `.at(k)` (no postfix `[]` operator exists). `{}` is the
// empty-BLOCK literal in Phalcom, not empty-map (spec §6) — `{}` ->
// `Map.new()`. List literals, nested `for`, string `+` concatenation all
// port directly.
//
// PARSER BUG found porting this file: a multi-line list literal with no
// trailing comma before the closing `]` fails to parse — `[a, b\n]` is
// "Expected ]", but `[a, b,\n]` (trailing comma) parses fine. Every
// multi-line list below (adverbs/adjectives/animals) carries an added
