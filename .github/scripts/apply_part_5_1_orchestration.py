...
text = path.read_text()
start = text.index("pub fn synthesize_match_expr(")
end = text.index("/// Returns the exact-case union established by a successful root pattern.", start)
current = text[start:end]

if "let mut remaining_space = initial_space.clone().normalize();" in current:
    print("ordered match orchestration already applied")
    raise SystemExit(0)

replacement = r'''pub fn synthesize_match_expr(ctx: &mut CheckingContext<'_>, match_expr: &phalcom_ast::ast::MatchExpr, expected: &ExpectedType) -> TypedExpression {
    ...
    for (arm_index, arm) in match_expr.arms.iter().enumerate() {
        ...
        let (pattern_res, arm_space) =
            crate::checker::pattern::resolve_pattern(ctx, &arm.pattern, scrutinee_ty, &initial_space, &mut arm_bindings);
        let (reachable, residual_after, usefulness) = crate::checker::exhaustiveness::evaluate_match_arm_usefulness(
            ctx,
            &initial_space,
            &remaining_space,
            &arm_space,
...
print("applied ordered match orchestration")
