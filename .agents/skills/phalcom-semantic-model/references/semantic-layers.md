...
## Bridge contracts between layers

A layer boundary is not merely a Rust module boundary. It is a semantic function whose
inputs, outputs and information loss should be explainable. Write bridges explicitly:

```text
resolve_name:
  (SourceOccurrence, ScopeGraph, ModuleGraph) -> Resolution<SemanticTarget>

infer_receiver_shape:
  (SemanticTarget, ProgramPoint, FlowFacts, Summaries) -> ShapeFact

resolve_dispatch:
  (ShapeFact, CanonicalSelector, DispatchContext, ClassSurfaces) -> DispatchResult

synthesize_type:                      FUTURE / typing-owned
  (ResolvedExpr, semantic facts, Γ) -> TypeResult

prove_obligation:                     FUTURE / prover-owned
  (VC, semantic/type/effect facts) -> Proved | Refuted | Unknown
```

Never hide a cross-domain conversion in a convenience accessor such as
`semantic_type_of_value()` unless its contract states whether it returns a runtime shape,
canonical language type, heuristic type candidate, or proof-refined type.

## Coherence does not require one giant enum

"One semantic truth" means one coherent ownership model, not one monolithic data type. A
published generation can contain several typed products:
...
Snapshot G
  ...
  ├─ language type facts             FUTURE
  └─ proof/effect facts               FUTURE
```

Coherence comes from shared semantic identities, dependency versions and one publication
boundary. A `TypeFact` and `ShapeFact` can disagree in a way that becomes a diagnostic; they
should not be forced into one variant simply to avoid that possibility.

## Worked cross-layer example

Consider conceptually:

```phalcom
import "./geometry" as G
let shape = G.Factory.new().make()
shape.draw()
```

A disciplined pipeline is:

```text
source layer
  identifies ranges for `G`, `Factory`, `make()`, `shape`, `draw()`

scope/resolution layer
  `G` -> import BindingId -> target ModuleId
  `shape` declaration/use -> same BindingId

surface layer
  target module exposes class Factory and canonical members

local/interprocedural shape layer
  call Factory.new().make() -> resolved callable summary -> possible result shape

flow layer
  stores result shape for BindingId(shape) at the later send program point

dispatch layer
  receiver shape + canonical draw() selector -> target/candidates/dynamic result

type layer (future)
  may synthesize/check a language type for the expression independently of ValueShape

consumer layer
  hover renders facts; definition navigates target; checker enforces type contract;
  optimizer only uses dispatch assumptions with validity strong enough for execution
```

If completion independently re-walks the import, guesses `Factory`, and performs a different
member lookup, the architecture has already forked semantic truth.

## Layer ownership under incomplete source

Recovery does not erase the pipeline. A half-written send can have:

```text
exact receiver BindingId
exact current flow fact
partial selector family
no exact dispatch target yet
```

Preserve the facts that remain justified. Do not convert one missing selector token into a
file-wide `Unknown` class surface. Conversely, do not invent a complete selector merely to
make completion easier; the consumer can work from a partial semantic query.

## Layer ownership under future typing

When typing arrives, avoid this migration failure:

```text
old: shape inference owns name resolution + dispatch
new: checker independently owns name resolution + typed member lookup
```

Instead:

```text
shared identity/resolution/surface/dispatch semantics
            ├─ advisory ValueShape analysis
            └─ normative type checker
```

Typing can introduce new facts and relations without rebuilding the lower semantic world.
If typing eventually participates in an explicitly ratified dispatch feature, that feature
must add a specified bridge/dispatch layer rather than mutating ordinary selector identity
implicitly.
