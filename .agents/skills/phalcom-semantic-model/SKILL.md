...
description: >-
  Use when designing or reviewing Phalcom semantic identities, resolution,
  runtime-value knowledge, flow facts, dispatch facts, callable summaries,
  module/project dependencies, incremental invalidation, provenance, semantic
  queries, or bridges from shared semantics into typing, proving, LSP, runtime
  contracts, reflection, diagnostics, refactoring, and optimization.
...
This skill owns **semantic doctrine**: what semantic identities/facts mean, how domains
relate, what uncertainty/provenance must survive, and how one coherent semantic truth is
shared across consumers. It intentionally does not become a catch-all implementation book.

Use neighboring skills for their deeper ownership when available:

- `semantic-analysis-development`: concrete Rust changes to the current semantic engine,
  source walkers, flow/CFG machinery, fixed-point implementation, queries and performance;
- `type-theory`: formal type relations, substitution, polymorphism, variance, inference and
  subtyping mathematics;
- `phalcom-typed-language`: Phalcom's normative concrete type-language semantics;
- `type-checker-development`: constraint generation/solving, checking algorithms and checker
  diagnostics in the repository;
- `static-prover-development`: VC generation, symbolic execution, SMT, contracts and proof
  engineering;
- `parser-development`: grammar, recovery, AST/source fidelity and parser implementation;
- `rust-compiler-engineering`: Rust ownership/arena/interning/performance implementation
  patterns beyond the semantic contracts defined here;
- `lsp-development`: protocol adapters, request/cancellation behavior and editor UX over
  semantic queries;
- language/object-model/module/concurrency skills and normative specs for feature semantics.

When a task crosses boundaries, this skill supplies the shared semantic invariants while the
neighbor supplies its specialized algorithm or implementation technique. Do not duplicate a
second textbook here.
...
Before writing a design or repository-specific claim, classify it explicitly when status
matters:

```text
CURRENT       observed in current implementation/tests
NORMATIVE     current ratified/spec behavior
PROPOSED      documented design not yet current behavior
EXPERIMENTAL  repository experiment without normative guarantee
FUTURE        expected/planned direction
RECOMMENDATION this skill's design advice
```

Never promote a proposed typing/module/concurrency design to current behavior. Conversely,
do not treat current implementation accidents as normative semantics without checking the
specification/decision record.

## Operational workflow

For semantic work, follow this order before writing Rust:

1. **State the semantic question.** Say exactly what entity/fact/query must mean.
2. **Classify authority/status.** Separate current implementation, normative rules and desired
   future behavior.
3. **Inspect owners.** Read the governing spec plus current AST/compiler/runtime/semantic
   implementation and tests relevant to the question.
4. **Name the observable invariant.** Define what Phalcom code/tools may observe and what is
   merely representation.
5. **Choose identity.** Define equality, qualification, lifetime and cross-edit stability.
6. **Choose domain.** Define concrete meaning, uncertainty states, precision order, join,
   bottom/top and widening where needed.
7. **Define dynamic/static bridge.** State how actual runtime semantics are approximated,
   typed, checked, proved, reflected and potentially optimized.
8. **Define flow/interprocedural behavior.** Identify program points, dependencies, fixed
   points, effects, recursion and abrupt completion.
9. **Define provenance.** Preserve enough causal evidence for diagnostics/debugging before
   flattening results.
10. **Define incrementality.** Record contribution ownership, dependency edges, invalidation,
    retraction, generation validity and cache bounds.
11. **Define recovery.** Separate malformed-source recovery facts from genuine semantic
    uncertainty/errors.
12. **Design discriminating tests.** Include negative, loop/recursive, incremental retraction,
    malformed-source and full-vs-incremental equivalence tests where relevant.
13. **Implement through the shared owner.** Consumers adapt semantic queries; they do not
    reinvent inference.
14. **Verify cross-consumer consistency and performance.** Compare compiler/runtime, checker,
    LSP and optimizer/prover assumptions according to feature scope.

If the task cannot answer steps 4-10, it is not ready to jump from feature prose to code.
...
- `facts.rs`: `ValueShape` (currently bounded to eight union alternatives), confidence,
  provenance, local/field/parameter facts, and contribution-indexed parameter evidence that
  supports retraction after edits;
...
- `infer.rs`: dependency-driven interprocedural inference/worklists and bounded operational
  iteration;
...
Use these as compact review equations:

```text
identity:
  spelling -> resolution -> SemanticId

flow:
  IN[B]  = join(OUT[p] for reachable predecessors p)
  OUT[B] = transfer_B(IN[B])

interprocedural:
  changed summary -> enqueue semantic dependents -> solve to stability

incremental edit:
  database' = database - old contribution + new contribution
  then propagate changed derived facts

semantic publication:
  mutable worker -> coherent immutable generation -> consumer queries

knowledge bridge:
  runtime shape != language type != proposition != optimizer assumption

dispatch:
  receiver + canonical selector + lookup context -> target/candidates/dynamic

proof:
  timeout / unsupported / no counterexample != proved
```

The first question for a cache is not "will it be fast?" but:

```text
key, value, dependencies, validity, invalidation, concurrency, memory bound
```

The first question for a new ID is:

```text
Two IDs are equal exactly when ... ?
```

The first question for `Unknown` is:
...
Read [references/abstract-knowledge.md](references/abstract-knowledge.md),
[references/identities-and-resolution.md](references/identities-and-resolution.md), and
[references/modules-and-incrementality.md](references/modules-and-incrementality.md) for
the implementation-grade reasoning behind these models.
...
A semantic change is not complete because one hover/completion fixture passes. Verification
should establish the semantic distinction at the lowest shared layer first, then the relevant
consumer behavior. Depending on scope, require:

```text
identity/resolution tests
abstract-domain join/fixed-point tests
branch + loop + recursion tests
malformed/incomplete source tests
module/provider creation/removal tests
incremental evidence-retraction tests
clean full rebuild == incremental final facts
runtime/compiler conformance for dynamic behavior
consumer integration tests
determinism/property/metamorphic tests
performance/rebuild-frontier regression guards
```

Use the application scenarios in
[references/review-checklist.md](references/review-checklist.md) as pressure tests. In
particular, an agent must reject: `ValueShape` renamed into the type system, one-pass loop
analysis, append-only cross-edit joins, range-as-stable-ID, implicit type-directed ordinary
dispatch, unguarded reflection-sensitive inlining, global poisoning from parser recovery, and
solver timeout presented as proof.
