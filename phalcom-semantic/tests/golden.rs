//! Isolated Plan 3 semantic golden-fixture binary.
//!
//! This target intentionally lives beside `semantic.rs` while the larger
//! semantic test tree is being expanded in parallel. It owns only the ten
//! programs from `03-ten-golden-semantic-fixtures.md` and can be run with:
//!
//! `cargo test -p phalcom-semantic --test golden`

#[path = "semantic/support/fixture.rs"]
mod fixture;
#[path = "semantic/support/workspace.rs"]
#[allow(dead_code)]
mod workspace;

use fixture::{Fixture, assert_source_contract, binding, known, union};
...
use std::path::Path;
use workspace::WorkspaceFixture;
...
fn golden_sources_are_local_to_this_binary() {
    ...
    assert!(manifest_dir.join("tests/golden.rs").is_file());
    assert!(manifest_dir.join("tests/golden").is_dir());
}
