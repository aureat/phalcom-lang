...
fn adt_incr_match_analysis_records_enum_and_callable_dependencies() {
    ...
    assert!(update.snapshot.callable_analyses.values().any(|callable| !callable.match_resolutions.is_empty()));
    ...
    let dependencies = session.db().index().dependencies_of(&QueryKey::CallableBody(callable)).expect("match body dependencies");
    assert!(dependencies.iter().any(|edge| edge.dependency == QueryKey::EnumDeclaration(DeclarationId::new(module.clone(), "Choice".into()))));
}
