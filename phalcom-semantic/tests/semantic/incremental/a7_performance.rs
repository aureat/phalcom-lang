...
fn a6_high_fanout_recomputes_exact_consumers_not_reverse_importers() {
    ...
    let updated = session.update(high_fanout_input("class Provider { @class value() -> String { \"changed\" } }\nexport Provider\n", 2));
    ...
    // enter semantic work.
    assert!(updated.stats.reverse_candidates_considered <= 350, "exact semantic closure exceeded the 100-consumer bound: {:?}", updated.stats);
    assert_eq!(updated.stats.callable_bodies_recomputed, 101, "provider plus the 100 exact consumers should recompute");
    assert!(updated.stats.semantic_dependents_recomputed <= 350, "semantic recomputation must remain proportional to exact consumers: {:?}", updated.stats);
    assert!(updated.stats.exact_name_products_recomputed <= 10, "name products must remain bounded: {:?}", updated.stats);
}
...
fn a6_high_fanout_unused_provider_body_edit_has_no_external_semantic_recompute() {
    ...
    assert!(!updated.snapshot.has_errors(), "body-only provider edit should remain valid: {:?}", updated.snapshot.diagnostics);
    assert_eq!(updated.stats.callable_bodies_recomputed, 1, "only the provider body should recompute");
    assert!(updated.stats.reverse_candidates_considered <= 20, "unused provider body must not traverse reverse importers: {:?}", updated.stats);
}
...
fn a6_high_fanout_unrelated_declaration_and_hierarchy_retain_existing_products() {
    ...
    assert!(!updated.snapshot.has_errors(), "unrelated declaration should remain valid: {:?}", updated.snapshot.diagnostics);
    assert_eq!(session.db().query_state(&provider_signature_key).and_then(|state| state.revision()), signature_revision);
    assert_eq!(session.db().query_state(&consumer_body_key).and_then(|state| state.revision()), consumer_body_revision);
    assert_eq!(session.db().query_state(&provider_hierarchy_key).and_then(|state| state.revision()), hierarchy_revision);
    assert_eq!(updated.stats.callable_signatures_recomputed, 0, "existing callable signature must be retained");
    assert_eq!(updated.stats.hierarchy_edges_recomputed, 1, "only the added declaration hierarchy edge should recompute");
}

#[test]
fn pa9_large_hierarchy_edit_isolates_unrelated_formal_products() {
    let provider_v1 = "class BaseA { @class value() -> Int { 1 } }\nclass BaseB { @class value() -> String { \"b\" } }\nclass Provider is BaseA {}\nexport Provider\n";
    let provider_v2 = "class BaseA { @class value() -> Int { 1 } }\nclass BaseB { @class value() -> String { \"b\" } }\nclass Provider is BaseB {}\nexport Provider\n";
    ...
    let stable_callable = CallableId::new(
        stable_declaration.clone(),
    ...
    let stable_revisions = stable_keys
        .iter()
        .map(|key| (key.clone(), incremental.db().query_state(key).and_then(|state| state.revision()).expect("initial stable product")))
    ...
    assert!(updated.snapshot.has_errors(), "the 100 exact consumers should observe BaseB's String method: {:?}", updated.snapshot.diagnostics);
    assert_eq!(updated.stats.semantic_structure_shards_recomputed, 1, "only the provider shard changes");
    assert_eq!(updated.stats.semantic_structure_shards_reused, 5_000, "all reverse-connected importer shards remain retained");
    assert!(updated.stats.reverse_candidates_considered <= 350, "hierarchy propagation must remain bounded by exact consumers: {:?}", updated.stats);
    assert!(updated.stats.callable_bodies_recomputed >= 100, "all exact hierarchy consumers must recompute: {:?}", updated.stats);
    ...
    assert_ne!(
        incremental.db().query_state(&QueryKey::CallableBody(consumer_body)).and_then(|state| state.revision()),
    ...
    for (key, revision) in stable_revisions {
        ...
        assert_eq!(state.revision(), Some(revision), "unrelated product {key:?} must retain its computation revision");
    }
    ...
    if updated.snapshot.hierarchy.superclasses != cold_result.snapshot.hierarchy.superclasses {
        ...
        let changed = incremental_hierarchy
            ...
            .count();
        let missing_keys = cold_hierarchy.keys().filter(|key| !incremental_hierarchy.contains_key(*key)).take(8).collect::<Vec<_>>();
        panic!("incremental and cold hierarchy results differ: incremental={}, cold={}, missing={missing}, extra={extra}, changed={changed}, missing_keys={missing_keys:?}", incremental_hierarchy.len(), cold_hierarchy.len());
    }
}
...
fn pa10_cold_and_incremental_snapshots_have_presentation_parity_across_mutations() {
    ...
    let mut incremental = SemanticWorkspaceSession::new();
    let initial = incremental.update(parity_input(0, 1));
    let initial_projection = semantic_parity_projection(&initial.snapshot);
    let mut cold = SemanticWorkspaceSession::new();
    assert_eq!(initial_projection, semantic_parity_projection(&cold.update(parity_input(0, 1)).snapshot));

    for step in 1..=LAST_STEP {
        let input = parity_input(step, step as u64 + 1);
        let incremental_result = incremental.update(input.clone());
        let cold_result = cold.update(input);
        assert_eq!(
            semantic_parity_projection(&incremental_result.snapshot),
            semantic_parity_projection(&cold_result.snapshot),
...
fn semantic_parity_projection(snapshot: &SemanticSnapshot) -> SemanticParityProjection {
    ...
    let mut surfaces = snapshot
        ...
        .surfaces()
        .iter()
        .map(|(declaration, surface)| format!("{declaration:?}|instance={}|class={}", member_surface_key(snapshot, &surface.instance), member_surface_key(snapshot, &surface.class)))
        .collect::<Vec<_>>();
    surfaces.sort();
    ...
    let mut hierarchy = snapshot
        ...
        .chain(snapshot.hierarchy.templates.iter().map(|(declaration, template)| {
            format!("template:{declaration:?}:{}", template.structural_form.as_deref().unwrap_or("-"))
        }))
        .collect::<Vec<_>>();
    hierarchy.sort();
    ...
    let mut source_index = snapshot
        ...
        .map(|(module, index)| {
            ...
            format!("{module:?}|semantic={:?}|presentation={:?}|sites={}|occurrences={}|attachments={}", fingerprints.semantic, fingerprints.presentation, index.structure.sites.len(), index.occurrences.all().len(), index.attachments.len())
        })
        .collect::<Vec<_>>();
    source_index.sort();
    ...
    let guidance = diagnostic
        ...
        .map(|guidance| match guidance {
            ...
            DiagnosticGuidance::EstablishTypeEvidence { range, expected } => {
                format!("evidence:{range:?}:{}", expected.map_or_else(|| "-".to_string(), |ty| snapshot.store.format_type(ty)))
            }
...
fn parity_input(step: usize, generation: u64) -> SemanticWorkspaceInput {
    ...
    let broken = if step == 14 { "\nclass Broken { @class value() -> Int { \"x\" } }" } else if step == 15 { "\nclass Broken { @class value() -> Int { 1 } }" } else { "" };
    ...
    for (module_id, source) in source_units {
        let program = Arc::new(phalcom_ast::parse(&source, 0).program);
        sources.insert(module_id.clone(), Arc::new(ParsedModuleUnit::new(module_id, ModuleKind::Module, None, source, program)));
    }
    ...
    let export = |owner: &ModuleId, name: &str| {
        (
            ...
            LinkedExport {
                public_name: name.into(),
                target: LinkedExportTarget::Binding(SymbolId { module: owner.clone(), name: name.into() }),
                range: phalcom_common::range::SourceRange::default(),
            },
        )
    };
    ...
    let mut provider_globals = BTreeMap::from([("Base".into(), GlobalBindingId(0)), ("Alt".into(), GlobalBindingId(1)), ("Alias".into(), GlobalBindingId(2))]);
    if step == 2 {
    ...
    let b_reads = if step == 7 {
        vec![
            LinkedReadSpec::Binding(SymbolId { module: a.clone(), name: "Alt".into() }),
            LinkedReadSpec::Binding(SymbolId { module: a.clone(), name: "Alt".into() }),
        ]
    } else {
        vec![
            LinkedReadSpec::Binding(SymbolId { module: a.clone(), name: "Base".into() }),
            LinkedReadSpec::Binding(SymbolId { module: a.clone(), name: "Alt".into() }),
        ]
    };
    let b_module = LinkedModule {
        ...
        bindings: ModuleBindingLayout {
            local_globals: BTreeMap::from([("Child".into(), GlobalBindingId(0)), ("Consumer".into(), GlobalBindingId(1)), ("Local".into(), GlobalBindingId(2))]),
            imports: b_imports,
        },
