...
fn match_flow_07_variant_arm_publishes_stable_binding_refinement() {
    ...
    assert!(!matches!(handle.arm(0).resolution().reachable_space, phalcom_semantic::match_semantics::PatternSpaceSummary::Empty));
}
...
fn match_flow_08_family_arm_keeps_candidate_space_in_match_product() {
    ...
    let handle = case.only_match();
    assert!(!matches!(handle.arm(0).resolution().reachable_space, phalcom_semantic::match_semantics::PatternSpaceSummary::Empty));
    assert!(!matches!(handle.arm(0).resolution().residual_after, phalcom_semantic::match_semantics::PatternSpaceSummary::Empty));
}

#[test]
fn match_flow_09_bindings_are_arm_local() {
    let case = analyze_adt("enum Choice { @variant Left(_ value: Int) @variant Right }\nclass Test { run(_ value: Choice) { match value { Choice::Left(x) => x Choice::Right => 0 } } }\n");
    let handle = case.only_match();
