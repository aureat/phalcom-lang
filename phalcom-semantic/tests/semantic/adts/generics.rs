...
fn adt_gen_01_option_payload_substitutes_to_concrete_type() {
    ...
    let some = case.variant("Option", phalcom_common::selector::Selector::method("Some", [phalcom_common::selector::SelectorSlot::Positional]).expect("Some"));
    assert_eq!(signature.parameter_count(), 1);
    assert!(matches!(case.type_data(some.fields[0].declared_type.canonical_type().expect("payload")), phalcom_semantic::types::store::TypeData::Parameter(parameter) if *parameter == signature.parameters[0]));
}
...
fn adt_gen_02_result_parameters_are_independent_per_variant() {
    ...
    let ok = case.variant("Result", phalcom_common::selector::Selector::method("Ok", [phalcom_common::selector::SelectorSlot::Positional]).expect("Ok"));
    let err = case.variant("Result", phalcom_common::selector::Selector::method("Err", [phalcom_common::selector::SelectorSlot::Positional]).expect("Err"));
    assert!(matches!(case.type_data(ok.fields[0].declared_type.canonical_type().expect("Ok payload")), phalcom_semantic::types::store::TypeData::Parameter(parameter) if *parameter == signature.parameters[0]));
    assert!(matches!(case.type_data(err.fields[0].declared_type.canonical_type().expect("Err payload")), phalcom_semantic::types::store::TypeData::Parameter(parameter) if *parameter == signature.parameters[1]));
}
...
fn adt_gen_03_nested_generic_payload_keeps_application_shape() {
    ...
    let value = case.variant("Boxed", phalcom_common::selector::Selector::method("Value", [phalcom_common::selector::SelectorSlot::Positional]).expect("Value"));
    assert!(matches!(case.type_data(value.fields[0].declared_type.canonical_type().expect("nested payload")), phalcom_semantic::types::store::TypeData::Applied { .. }));
}
...
fn adt_gadt_01_case_environment_is_owned_by_specialized_variant() {
    ...
    let int = case.variant("Expr", phalcom_common::selector::Selector::method("Int", [phalcom_common::selector::SelectorSlot::Positional]).expect("Int"));
    let bool_case = case.variant("Expr", phalcom_common::selector::Selector::method("Bool", [phalcom_common::selector::SelectorSlot::Positional]).expect("Bool"));
