...
#[test]
fn inferred_local_formal_binding_attaches_to_exact_name_site() {
    ...
    let source_binding = module_index
        ...
        .bindings
        .values()
        .find(|binding| binding.name.as_ref() == "inferred" && binding.kind == SourceBindingKind::LocalConst)
        .expect("local source binding");
    let callable_analysis = analysis
        .snapshot
        .callable_analyses
        .values()
        .find(|callable| callable.bindings.values().any(|binding| binding.name == "inferred"))
        .expect("callable analysis");
    let formal_binding = callable_analysis
        .bindings
        .values()
        .find(|binding| binding.name == "inferred")
        .expect("formal local binding");
