...
fn formal_projection_preserves_causal_invalidity_and_binding_contracts() {
    ...
    assert!(
        update.snapshot.sources.contains_key(&module),
        "semantic errors must not drop source publication"
    );
    ...
    assert!(
        update.snapshot.source_index().module(&module).is_some(),
        "semantic errors must not drop source index"
    );
    assert!(
        update.snapshot.advisory().module(&module).is_some(),
        "semantic errors must not drop advisory products"
    );
    let callable = CallableId::new(
