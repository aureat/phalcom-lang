...
pub fn export_type_form(store: &TypeStore, form: TypeId) -> Result<CompiledTypeRef, SemanticExportError> {
    match store.get(form) {
        ...
        TypeData::Applied { origin, arguments } => {
            ...
            let args = arguments.iter().map(|&a| export_type_form(store, a)).collect::<Result<Vec<_>, _>>()?;
        ...
        TypeData::Union(members) => {
            let mems = members.iter().map(|&m| export_type_form(store, m)).collect::<Result<Vec<_>, _>>()?;
            Ok(CompiledTypeRef::Union(mems.into_boxed_slice()))
        }
        TypeData::Tuple(elems) => {
            let tuple_elems = elems
                .iter()
                .map(|e| export_type_form(store, e.ty).map(|t| CompiledTupleElement { label: e.label.clone(), ty: t }))
        ...
        TypeData::Callable(c) => {
            let pos = c
                .parameters
                .iter()
                .map(|p| export_type_form(store, p.ty).map(|t| CompiledCallableParam { name: p.label.clone(), ty: t }))
                .collect::<Result<Vec<_>, _>>()?;
