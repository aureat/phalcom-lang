...
**Apply.** Write the semantics down before optimizing the implementation; treat the spec + golden corpus as the oracle the VM must match. → overlay: `docs/spec/current/*` + ADRs are the source of truth; `verify_invariants()` and a golden `.ph` corpus enforce them against the runtime.
