...
- **kernel `List` is an unscheduled hard dependency** — `Message.args` (dNU) and rest-params both require `List`, which no unit builds yet and which collides with U-STD in the same wave. Status: **DEC-A ✅ resolved (land U-LIST first, before Wave F); storage design pinned by [ADR-0020](../../../../docs/adr/0020-kernel-list-native-array-protocol.md) (native `Vec<Value>` array + `.ph` protocol) — ADR-0020 + its dependency ADR-0019 are Proposed, ratification still pending.**
- **`static` naming collision** — ADR-0011 uses "static" for the instance slot layout, but class-side *stored* fields (also "static") are unspecified. Status: **DEC-D ✅ resolved (include in U7, option A) — shipped as [ADR-0017](../../../../docs/adr/0017-class-side-stored-static-fields.md) (Accepted): class object gets its own `static_slots`, indexed by the metaclass field table.**
