...
/// type `T`, or `Error(error)`, containing a failure value of type `E`.
///
/// Operations on a result generally act on one variant while propagating the
/// other. Success transformations preserve `Error` values, while error
...
/// @typeparam E The type of error carried by `Error`.
@native
enum Result<T, E> {
