...
#[test]
fn test_model_01_and_02_and_proj_and_name_project_hierarchy() {
    let mut vm = VM::new();
...
#[test]
fn test_model_03_standalone_package_has_no_project() {
    let mut vm = VM::new();
...
#[test]
fn test_model_04_standalone_module_has_no_package_and_no_project() {
    let mut vm = VM::new();
...
#[test]
fn test_root_01_02_04_builtin_client_and_std_resolution() {
    let mut vm = VM::new();
...
#[test]
fn test_root_03_standalone_context_rejects_user_project_dependencies() {
...
#[test]
fn test_core_01_legacy_core_import_rejected() {
...
fn test_pre_01_to_04_curated_prelude_and_shadowing() {
    let mut vm = VM::new();
    // PRE-01: Object, Class, Number, String, List, Map, Set, Option, Some, Function are in prelude
    for name in ["Object", "Class", "Number", "String", "List", "Map", "Set", "Option", "Some", "Function"] {
        let sym = vm.interner.intern(name);
        assert!(vm.prelude_names.contains(&sym), "prelude must contain {name}");
    }
    // PRE-02: Behavior, Metaclass, Selector are NOT in prelude
    for name in ["Behavior", "Metaclass", "Selector", "Message"] {
        let sym = vm.interner.intern(name);
...
#[test]
fn test_proj_01_to_04_manifest_validation() {
    let tmp = TempDir::new().unwrap();
...
#[test]
fn test_ref_01_and_02_dunder_policy_and_internal_floor() {
...
#[test]
fn test_meta_01_to_03_unit_metadata_fixture() {
...
#[test]
fn test_scc_01_semantic_scc_fixture_realizes() {
