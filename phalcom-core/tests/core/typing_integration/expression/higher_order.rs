...
fn apply_preserves_function_argument_relationship() {
    ...
    let function_args = f.assert_applied(function, "Expression", 2);
    let function_type = function_args[1];
    let TypeData::Callable(signature) = f.analysis.snapshot.store.get(function_type) else {
        panic!("Apply function payload must be callable: {}", f.analysis.snapshot.store.format_type(function_type));
    };
    ...
    let argument_args = f.assert_applied(argument, "Expression", 2);
    ...
    let call = f.expression_containing(run, "ExpressionEvaluation.eval(monad, expression)");
    let target = f.callable_id("ExpressionEvaluation", "eval", DispatchSide::Class);
    f.assert_expression_call(call, &target, f.assert_binding_applied(run, "result", "Either", &[super::super::support::nominal("String"), super::super::support::nominal("Bool")]));
}
