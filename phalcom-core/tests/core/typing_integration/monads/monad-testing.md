# Walkthrough — `phalcom-core/tests/core/typing_integration/monads`
...
The focused Monad sub-suite is:
...
```text
phalcom-core/tests/core/mod.rs (via `typing_integration/mod.rs`)
```
...
The canonical fixture declaration lives at
`phalcom-core/tests/core/typing_integration/sources/either.ph` and is composed
into the Monad source by the shared root harness.
...
# 8. root `support.rs` — semantic test harness

Root `support.rs` is a major part of package and shared with direct Either
tests.
...
Root `LAWS.md` is the authoritative inventory.
...
That makes `phalcom-core/tests/core/typing_integration` a conformance package
not merely for higher-kinded syntax, but for the interaction between
Phalcom's declaration model, canonical type representation, inference engine,
proof/explanation machinery, inheritance specialization, and runtime
execution.
