# Unified Typing Integration Laws — GEN, MON, GEX, and INT

This document is authoritative catalog for laws exercised by
`phalcom-core/tests/core/typing_integration`.

Package uses direct `Either` and `Functor`/`Applicative`/`Monad` abstractions as
pressure vessels for Phalcom's type system. It does **not** claim to prove
category-theoretic laws. Purpose: prove constructor-kinded generics, type
lambdas, inheritance, generic method inference, proof provenance, and runtime
execution compose correctly.
...
## `GEN-*` — generic ADT authority

GEN law definitions remain authoritative in
[`docs/spec/typing-generics.md`](../../../../docs/spec/typing-generics.md).
Direct Either scenarios under `either/` preserve those identifiers and
exercise canonical declaration at `sources/either.ph`. This catalog does not
duplicate or renumber GEN specification.

## `GEX-*` — typed Expression GADT integration

### GEX-01 — indexed literal branches preserve exact effect results

`Expression::IntLiteral` and `Expression::BoolLiteral` refine `T`, allowing
`ExpressionEvaluation.eval<F,T>` to return exact `F<Int>` and `F<Bool>`.

Test: `expression::refinement::gadt_literal_branches_preserve_exact_effect_index`

### GEX-02 — constructor-local higher-order variables remain fresh

`Map` and `FlatMap` keep their constructor-local `A` and `B` relationships
separate from evaluator and call-site variables.

Test: `expression::higher_order::map_preserves_callable_input_and_result_indices`
Test: `expression::higher_order::flat_map_preserves_continuation_result_index`

### GEX-03 — GADT equality propagates through HKT application

Branch-local equality such as `T = Int` is consumed inside `F<T>` without
widening the result to `Dynamic` or an unexplained `Unknown`.

Test: `expression::refinement::gadt_literal_branches_preserve_exact_effect_index`

### GEX-04 — Apply preserves function/argument index agreement

`Expression<F,(A) -> B>` combined with `Expression<F,A>` constructs exact
`Expression<F,B>` and preserves the callable payload signature.

Test: `expression::higher_order::apply_preserves_function_argument_relationship`

### GEX-05 — FlatMap composes recursive evaluation with Monad.flatMap

The continuation result index remains exact through recursive `eval` and the
existing `Monad<F>.flatMap` contract.

Test: `expression::higher_order::flat_map_preserves_continuation_result_index`

### GEX-06 — captured Expression constructor has unary kind

`<X> =>> Expression<F, X>` is a canonical `Type -> Type` lambda that captures
outer `F` while retaining lambda-bound `X`.

Test: `expression::monad::expression_monad_captures_effect_constructor_and_projects_hierarchy`

### GEX-07 — ExpressionMonad projects through existing Monad hierarchy

`ExpressionMonad<F>` uses the existing `Functor`/`Applicative`/`Monad`
contracts with the exact Expression constructor and no duplicate algorithm
hierarchy.

Test: `expression::monad::expression_monad_captures_effect_constructor_and_projects_hierarchy`

## `INT-*` — Expression, Monad, and Either composition

### INT-00 — direct Either and Monad paths share one canonical declaration

One semantic analysis must resolve direct `Either.map` and
`MonadAlgorithms.bind` to exact callables while both results use the same
canonical `Either<String, Bool>` family and exact constructor inference.

Test: `integration::direct_either_and_monad_paths_share_one_canonical_source`

### INT-01 — concrete Either evaluation preserves exact indexed result

For `F = <X> =>> Either<String, X>`, evaluating `Expression<F,T>` yields exact
`Either<String,T>`.

Test: `expression::integration::traverse_expression_then_evaluate_preserves_exact_intermediate_and_final_types`

### INT-02 — existing traverse infers the Expression constructor

`MonadAlgorithms.traverse` constructs `Expression<F,List<B>>` through the
existing generic algorithm, with exact constructor and element solutions.

Test: `expression::monad::existing_traverse_infers_expression_constructor_and_exact_list_result`

### GEX-REJECT-01..05 — hostile indexed and higher-order constructors fail closed

Wrong Add/If indices, incompatible If branches, Apply input mismatch, and
FlatMap continuation mismatch are rejected with probe-local conflict/mismatch
diagnostics and never become `Dynamic`.

Tests: `expression::rejection::add_rejects_bool_operand_without_dynamic_escape`,
`expression::rejection::if_rejects_int_condition_without_dynamic_escape`,
`expression::rejection::if_rejects_incompatible_branch_indices_without_dynamic_escape`,
`expression::rejection::apply_rejects_callable_argument_mismatch_without_dynamic_escape`,
`expression::rejection::flat_map_rejects_continuation_input_mismatch_without_dynamic_escape`

### INT-REJECT-01 — effect constructors cannot be interchanged by kind alone

An `Expression<<X> =>> Either<String, X>, T>` cannot be evaluated with
`BoxMonad` merely because both constructors have unary kind.

Test: `expression::rejection::evaluator_rejects_effect_constructor_mismatch_without_dynamic_escape`

### INT-RUNTIME-00..05 — Expression VM behavior agrees with semantic preflight

Pure/add, higher-order Apply, Lift success, Lift failure short-circuiting, and
the traverse-to-eval flagship produce exact primitive observations after the
same source passes semantic analysis.

Tests: `expression::runtime::runtime_fixture_is_semantically_valid_before_execution`,
`expression::runtime::expression_runtime_surface_produces_expected_values`

### INT-03 — traverse-to-eval flagship preserves intermediate and final types

`List<Int>` traverses through `StringEitherExpressionMonad` to
`Expression<<X> =>> Either<String, X>, List<Int>>`, then evaluates through
`StringEitherMonad` to `Either<String, List<Int>>`.
