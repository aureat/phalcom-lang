...
const INTEGRATION_PROBES: &str = include_str!("sources/integration_probes.ph");
...
pub fn either_semantic_source() -> String {
    format!("{EITHER_SOURCE}\n{EITHER_SEMANTIC_PROBES}")
}

pub fn either_runtime_source() -> String {
    format!("{EITHER_SOURCE}\n{EITHER_RUNTIME_PROBES}")
}
...
pub fn monads_source() -> String {
    format!("{EITHER_SOURCE}\n{MONADS_SOURCE}")
}

pub fn monad_semantic_source() -> String {
    format!("{EITHER_SOURCE}\n{MONADS_SOURCE}\n{MONAD_SEMANTIC_PROBES}")
}

pub fn monad_runtime_source() -> String {
    format!("{EITHER_SOURCE}\n{MONADS_SOURCE}\n{MONAD_RUNTIME_PROBES}")
}
...
pub fn integration_semantic_source() -> String {
    format!("{EITHER_SOURCE}\n{MONADS_SOURCE}\n{INTEGRATION_PROBES}")
}

pub fn expression_source() -> String {
    format!("{EITHER_SOURCE}\n{MONADS_SOURCE}\n{EXPRESSION_SOURCE}")
}

pub fn expression_semantic_source() -> String {
    format!("{EITHER_SOURCE}\n{MONADS_SOURCE}\n{EXPRESSION_SOURCE}\n{EXPRESSION_SEMANTIC_PROBES}")
}

pub fn expression_runtime_source() -> String {
    format!("{EITHER_SOURCE}\n{MONADS_SOURCE}\n{EXPRESSION_SOURCE}\n{EXPRESSION_RUNTIME_PROBES}")
}
...
impl Fixture {
    ...
    pub fn assert_type(&self, actual: TypeId, expected: &Ty<'_>) {
        ...
        match expected {
            Ty::Nominal(name) => self.assert_nominal(actual, name),
            Ty::Applied(name, args) => {
                let actual_args = self.assert_applied(actual, name, args.len()).to_vec();
                for (actual, expected) in actual_args.into_iter().zip(args) {
