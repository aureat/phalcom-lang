...
pub(crate) struct Compiler<'vm> {
    ...
    /// Optional closed-link namespace supplied by a program compiler.
...
impl<'vm> Compiler<'vm> {
    ...
    /// module. Qualified references are resolved by the VM-free linker and
    /// must be materialized into a runtime class map before bytecode lowering.
    ///
    ...
    /// Resolves the runtime class key for a source superclass reference when
    /// it is local/bare. Qualified references remain linker identities until
    /// runtime declaration materialization; treating their leaf as local would
    /// silently bind the wrong class.
    pub(crate) fn resolve_superclass_ref(&self, reference: &phalcom_ast::ast::StaticSymbolRef) -> Option<ClassKey> {
        reference
            .is_bare()
            .then(|| self.vm.interner.find(&reference.root))
            .flatten()
            .and_then(|name| self.resolve_superclass_key(name))
    }
    ...
    fn predeclare_known_globals(&mut self, program: &Program) {
        ...
        if let Some(bindings) = &self.linked_bindings {
