...
impl VM {
    ...
    pub fn error_kind_symbol(&mut self, err: &RuntimeError) -> Option<Value> {
        let sym_str = match err {
            ...
            RuntimeError::SelectorPatternMismatch(_) => "selectorPatternMismatch",
            RuntimeError::UnsupportedOperation(_) => "unsupportedOperation",
            RuntimeError::InvalidCompareReturn { .. } => "invalidCompareReturn",
            RuntimeError::StrictZipLengthMismatch => "strictZipLengthMismatch",
            RuntimeError::PatternMismatch => "patternMismatch",
