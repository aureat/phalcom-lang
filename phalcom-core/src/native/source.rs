...
impl NativeSourceIndex {
    ...
    pub fn build() -> Result<Self, String> {
        ...
        for node in provider.nodes() {
            ...
            index.index_program(&parsed.program)?;
        }
    ...
    fn index_program(&mut self, program: &Program) -> Result<(), String> {
        for stmt in &program.statements {
            if let Statement::Class(class_def) = stmt {
                self.index_class(class_def)?;
            }
        }
        Ok(())
    }

    fn index_class(&mut self, class_def: &ClassDef) -> Result<(), String> {
        ...
        for member in &class_def.members {
            self.index_member(owner_key, &class_def.name, member)?;
        }

        Ok(())
    }

    fn index_member(&mut self, owner_key: UniverseKey, class_name: &str, member: &ClassMember) -> Result<(), String> {
        let (is_native, has_internal, is_internal, side, selector, is_declaration, range) = match member {
            ClassMember::Method(m) => {
                ...
                (is_native, has_internal, is_internal, side, selector, is_declaration, m.range)
            }
            ClassMember::Getter(g) => {
                ...
                (is_native, has_internal, is_internal, side, selector, is_declaration, g.range)
            }
            ClassMember::Setter(s) => {
                ...
                (is_native, has_internal, is_internal, side, selector, is_declaration, s.range)
            }
            ClassMember::Field(f) => {
                ...
                (is_native, has_internal, is_internal, side, selector, true, f.range)
            }
            ClassMember::Index(ix) => {
                ...
                (is_native, has_internal, is_internal, side, selector, false, ix.range)
            }
