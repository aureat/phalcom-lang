...
/// `ClassMember::Field(_) => MemberKind::Getter` arm for declared `let`/`var`
/// class fields (U-ANNOT-LAYOUT §3.1) previously lived here, but
/// `phalcom_ast`'s `ClassMember` on this branch has no `Field` variant —
/// U-ANNOT-LAYOUT has not landed on `phalcom-ast` at this commit, which made
/// this crate fail to compile at HEAD before Stage 4. Re-add the arm
/// (rendering like a getter — bare name, no parens, no synthesized accessor
/// yet) once `ClassMember::Field` lands upstream.
    ...
    fn walk_class(&mut self, class_def: &ClassDef) {
        ...
        for member in &class_def.members {
            ...
            match member {
                ...
                // NOTE (pre-existing conflict, fixed in passing by U-LSP
                // Stage 4): a `ClassMember::Field(f) => { ... }` arm for
                // declared `let`/`var` class fields (U-ANNOT-LAYOUT §3.1)
                // previously lived here, but `phalcom_ast`'s `ClassMember`
                // on this branch has no `Field` variant — U-ANNOT-LAYOUT has
                // not landed on `phalcom-ast` at this commit, which made
                // this crate fail to compile at HEAD before Stage 4. Re-add
                // the arm (indexing the field's bare name, walking its
                // optional default-value expression for references) once
