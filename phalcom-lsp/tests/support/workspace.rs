...
impl TestWorkspace {
    pub fn from_fixture_dir(relative: impl AsRef<Path>) -> Self {
        ...
        let root = std::env::temp_dir().join(format!("phalcom-lsp-test-{}-{id}", std::process::id()));
    ...
    pub fn uri(&self) -> String {
        Url::from_directory_path(&self.root).expect("workspace path can become file URL").to_string()
    }
...
fn copy_tree(source: &Path, destination: &Path) {
    for entry in fs::read_dir(source).unwrap_or_else(|err| panic!("read fixture directory {}: {err}", source.display())) {
